-- fix(chat): get_chat_rooms_for_user auth guard
--
-- 비인증 상태에서 조용히 빈 배열을 반환하던 것을
-- RAISE EXCEPTION으로 변경하여 클라이언트에서 에러를 감지할 수 있게 합니다.
-- 클라이언트 측 enabled 가드(chatRoomsQueryOptions의 userId 파라미터)가
-- 먼저 배포되어야 하며, 그래야 auth 미복구 시 쿼리 자체를 막아줍니다.
