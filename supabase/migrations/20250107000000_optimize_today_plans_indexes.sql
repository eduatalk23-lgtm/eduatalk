...
CREATE INDEX IF NOT EXISTS idx_student_plan_tenant_student_date_group
ON public.student_plan(tenant_id, student_id, plan_date, plan_group_id)
WHERE tenant_id IS NOT NULL;

-- 1-2. tenant_id가 없는 경우를 위한 인덱스
CREATE INDEX IF NOT EXISTS idx_student_plan_student_date_group_null_tenant
...
CREATE INDEX IF NOT EXISTS idx_student_plan_student_date_block_include
...
CREATE INDEX IF NOT EXISTS idx_study_sessions_student_plan_ended
ON public.student_study_sessions(student_id, plan_id, ended_at)
WHERE ended_at IS NULL;

-- 2-2. fullDaySessions 최적화: started_at range query 지원
CREATE INDEX IF NOT EXISTS idx_study_sessions_student_started_desc
