 ...
 */
export interface CalendarConfigCamel {
 ...
 *   .single();
 *
 * const config = inheritCalendarConfigFromRaw(calendar);
 * await supabase.from("plan_groups").insert({ ...config, ... });
 * ```
 */
export function inheritCalendarConfigFromRaw(
  calendar: CalendarConfigRaw
): PlanGroupCreationConfig {
  // lunch_time을 non_study_time_blocks에 통합
  const mergedBlocks = mergeLunchTimeIntoNonStudyBlocks(
    calendar.lunch_time,
    calendar.non_study_time_blocks
  );
  ...
  return {
    study_hours: calendar.study_hours ?? SCHEDULER_DEFAULTS.STUDY_HOURS,
    self_study_hours:
      calendar.self_study_hours ?? SCHEDULER_DEFAULTS.SELF_STUDY_HOURS,
    lunch_time: lunchTime,
    scheduler_type:
      calendar.default_scheduler_type ?? SCHEDULER_DEFAULTS.TYPE,
    scheduler_options:
      (calendar.default_scheduler_options as SchedulerOptions) ??
      SCHEDULER_DEFAULTS.OPTIONS,
    block_set_id: calendar.block_set_id ?? null,
...
/**
 * 캘린더 설정을 플랜 그룹 생성용 설정으로 변환 (Camel 형식)
 *
 * CalendarSettings 인터페이스 형식의 데이터를 플랜 그룹 생성에 필요한 형식으로 변환합니다.
 *
 * @param calendar - CalendarSettings 인터페이스 형식의 데이터 (camelCase)
 * @returns 플랜 그룹 생성용 설정
 */
export function inheritCalendarConfig(
  calendar: CalendarConfigCamel
): PlanGroupCreationConfig {
  // lunch_time을 non_study_time_blocks에 통합
  const mergedBlocks = mergeLunchTimeIntoNonStudyBlocks(
    calendar.lunchTime,
    calendar.nonStudyTimeBlocks
  );
  ...
  return {
    study_hours: calendar.studyHours ?? SCHEDULER_DEFAULTS.STUDY_HOURS,
    self_study_hours:
      calendar.selfStudyHours ?? SCHEDULER_DEFAULTS.SELF_STUDY_HOURS,
    lunch_time: lunchTime,
    scheduler_type:
      calendar.defaultSchedulerType ?? SCHEDULER_DEFAULTS.TYPE,
    scheduler_options:
      (calendar.defaultSchedulerOptions as SchedulerOptions) ??
      SCHEDULER_DEFAULTS.OPTIONS,
    block_set_id: calendar.blockSetId ?? null,
    non_study_time_blocks: mergedBlocks,
  };
}
