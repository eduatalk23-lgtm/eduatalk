...
export async function savePlanContents(
  ...
  try {
    ...
    const contentsInput: UpsertPlanContentInput[] = contents.map((c, index) => ({
      ...
      individual_schedule: (c.individual_schedule as unknown as Record<string, unknown>) ?? null,
      custom_study_days: (c.custom_study_days as unknown as number[]) ?? null,
      content_scheduler_options: (c.content_scheduler_options as unknown as Record<string, unknown>) ?? null,
      ...
      recommendation_metadata: (c.recommendation_metadata as unknown as Record<string, unknown>) ?? null,
