...
async function _addPlanExclusion(formData: FormData): Promise<void> {
  ...
  if (targetGroupId) {
    result = await createPlanExclusions(targetGroupId, tenantContext.tenantId, [
      exclusionData,
    ]);
  } else {
    ...
    if (!calendar?.id) {
      throw new AppError("캘린더를 찾을 수 없습니다.", ErrorCode.NOT_FOUND, 404);
    }

    result = await createStudentExclusions(
      calendar.id,
      user.userId,
      tenantContext.tenantId,
      [exclusionData],
