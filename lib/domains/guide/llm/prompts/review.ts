...
### 1-1. 과학적 정합성 (scientificAccuracy, 0~100)
- **개념 정확성**: 사용된 과학적 개념/용어가 정확한가. 개념 혼동이 없는가 (예: 항산화 능력 ≠ 영양학적 우수성)
- **탐구 전제-실험-결론 정합**: 탐구 질문(전제)과 실험 방법이 논리적으로 맞는가. 결론이 가설에서 자연스럽게 도출되는가
- **실험설계 타당성**: 비교군/대조군이 올바르게 설정되었는가. 변인 통제가 적절한가
- **결론 비자명성**: 과학적으로 당연한 결론을 발견처럼 포장하지 않았는가
- **출처 내용 일치**: 참고 도서/논문의 실제 내용과 가이드에서 인용한 내용이 일치하는가
- **별개 원리 구분**: 서로 다른 원리의 활동이 하나의 연속 탐구처럼 잘못 서술되지 않았는가
- 80점 이상: 개념 정확, 실험설계 타당, 결론 논리적
- 60~79점: 일부 비약이 있으나 치명적이지 않음
- 60점 미만: 심각한 개념 혼동 또는 논리적 비약
