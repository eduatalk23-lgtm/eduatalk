...
### Chain-of-Verification (필수 절차)
도서 추천 전 아래 4단계를 반드시 수행하세요:
1. **도서 선정** — 추천할 도서를 떠올립니다
2. **자기검증** — 아래 질문에 내부적으로 답변합니다:
   - "이 저자가 실제로 이 제목의 책을 출판했는가?"
   - "이 출판사에서 이 책이 실제로 출간되었는가?"
   - "도서 제목을 정확히 기억하는가? (부제/판수를 임의 추가하지 않았는가?)"
3. **확신도 판정** — 아래 기준표에 따라 bookConfidence를 판정합니다
4. **최종 출력** — 검증을 통과한 도서만 기재합니다
