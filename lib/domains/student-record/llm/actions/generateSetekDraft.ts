...
1. **지적호기심**: 수업에서 배운 개념에서 출발한 구체적 의문
2. **주제 선정**: 교과에서 자연스럽게 확장된, 학생 수준에 적합한 주제
3. **탐구 과정**: 단계적이고 논리적인 탐구 (실험/조사/분석 등)
4. **참고자료**: 독서나 자료가 탐구와 실제로 연결
5. **결론→행동**: 단순 "알게 되었다"가 아닌 해결방안/제언/창의적 고안
6. **교사 관찰**: 구체적 관찰 근거 기반 서술 ("특히, ~" 형식)
7. **성장 서사**: 성취과정/발전가능성이 드러남
8. **오류→재탐구**: 가능하면 한계 인식 후 추가 탐구 수행 (큰 가산)
...
- ❌ **나열식**: "수행평가에서 A를 하고, B를 하고, C를 함" — 활동만 나열하고 연결/흐름 없음
- ❌ **상투적 복붙**: "수업에 적극적으로 참여하며 성실한 태도를 보임" — 모든 학생에게 쓸 수 있는 표현
- ❌ **과도한 심화**: 고등학생 수준을 크게 벗어나는 대학원급 내용 (대리작성 의심)
- ❌ **인과 단절**: 실험 결과와 무관한 결론으로 갑자기 전환
- ❌ **자명한 결론**: 과학적으로 당연한 사실을 마치 발견처럼 포장
- ❌ **진로 과잉 도배**: 모든 교과에 동일 진로 키워드를 강제 삽입
- ❌ **교사 관찰 불가 표현**: "~다짐함", "~생각함", "~깨닫게 됨" (교사가 관찰할 수 없는 내면 상태)`;
...
export async function generateSetekDraftAction(
  ...
  try {
    ...
    if (input.rankGrade != null) {
      ...
      const level = grade9 <= 2 ? "심화+확장" : grade9 <= 4 ? "발전" : "기본";
