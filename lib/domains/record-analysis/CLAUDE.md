...
Grade Pipeline (학년별, 12태스크×9Phase)
  P0 (암시적): ctx.belief.profileCard 1회 빌드 (Layer 0, 2/3학년만)
  P1: competency_setek        ← ctx.belief.profileCard 주입 → ctx.belief.analysisContext에 축적
  P2: competency_changche     ← ctx.belief.profileCard 주입 → ctx.belief.analysisContext에 축적
  P3: competency_haengteuk    ← ctx.belief.profileCard 주입 → ctx.belief.analysisContext에 축적 + 집계
...
Layer 0 (P1-P3 진입 시 1회)
  └─ ctx.belief.profileCard: 이전 학년 competency_scores + content_quality 누적 프로필
...
Phase 1-3 (역량 분석)
  ...
  ├─ ctx.belief.analysisContext: 가공된 요약만 (issues있는 레코드 + B-이하 역량)
  │
...
| `buildStudentProfileCard()` / `renderStudentProfileCard()` | 이전 학년 역량/품질 집계 → Layer 0 프로필 카드 텍스트 | P1-P3 역량 분석 진입 시 (1회, `ctx.belief.profileCard` 캐시) |
