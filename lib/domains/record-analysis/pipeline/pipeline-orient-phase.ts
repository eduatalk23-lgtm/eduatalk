// ============================================
// Orient Phase — 규칙 기반 Planner (β(A) 재포지셔닝, 2026-04-24)
//
// 학생 상태(belief) 를 읽고 downstream Grade Phase 4~9 의
// skipTasks / modelTier 를 결정적(규칙 기반)으로 판정한다.
//
// β(A) 2026-04-24 변경:
// - LLM 호출 완전 제거 (runLlmPlanner / mergePlannerResults / safety 로직 삭제).
// - PlannerDirective 에서 LLM telemetry 필드
//   (plannerSource, llmRationale, llmDurationMs, recordPriorityOverride) 삭제.
// - LLM 메타 판정은 MidPlanner(P3.5 직후) 로 이동:
//   `pipeline/orient/mid-pipeline-planner.ts` + `ENABLE_MID_PIPELINE_PLANNER` env flag.
//
 ...
 *
 * β(A) 2026-04-24: LLM telemetry 필드 제거 (plannerSource / llmRationale / llmDurationMs /
 * recordPriorityOverride). LLM 메타 판정은 ctx.midPlan (MidPlanner) 로 분리.
 */
...
/**
 * Orient Phase 실행 — 규칙 기반 판정만 수행 (LLM 호출 0회).
 *
 * 규칙:
 ...
 *
 * LLM 메타 판정은 MidPlanner(ENABLE_MID_PIPELINE_PLANNER, P3.5 직후) 로 분리됨.
 */
