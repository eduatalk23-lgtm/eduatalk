...
import type { HighlightAnalysisInput, StepATaggingResult, TagWithUncertainty, SectionWithUncertainty, StepAPreAnalysis } from "../types";
...
0. **사전 착안점 (preAnalysis) 먼저 작성**: 태그 추출 전에 **반드시** 이 세특의 전체 그림을 파악하여 preAnalysis 필드에 기술합니다:
   - mainTheme: 이 기록의 핵심 주제·활동 (1~2문장)
   - careerLink: 목표 전공 또는 진로와의 연결도 (직접 연결/간접 연결/무관 중 선택 + 근거)
   - qualityConcerns: 사전 감지된 품질 우려 (나열식/성장부재/과잉도배/교사관찰불가 등 의심 신호). 우려 없으면 "없음"
   이 단계는 이후 태그 추출의 일관성 기준점이 됩니다. 각 태그의 reasoning은 preAnalysis와 모순되지 않아야 합니다.
...
{
  "preAnalysis": {
    "mainTheme": "수학 교과에서 자기주도 학습과 심화 탐구를 동시에 수행한 기록",
    "careerLink": "직접 연결 — 목표 전공(경제학)과 관련된 통계·모델링 요소 포함",
    "qualityConcerns": "없음"
  },
  "sections": [
    {
...
export function parseStepAResponse(content: string): StepATaggingResult {
  ...
  let preAnalysis: StepAPreAnalysis | undefined;
  const rawPre = parsed.preAnalysis;
  if (rawPre && typeof rawPre === "object") {
    const pa = rawPre as Record<string, unknown>;
    const mainTheme = typeof pa.mainTheme === "string" ? pa.mainTheme.trim() : "";
    const careerLink = typeof pa.careerLink === "string" ? pa.careerLink.trim() : "";
    const qualityConcerns = typeof pa.qualityConcerns === "string" ? pa.qualityConcerns.trim() : "";
    if (mainTheme || careerLink || qualityConcerns) {
      preAnalysis = { mainTheme, careerLink, qualityConcerns };
    }
  }
