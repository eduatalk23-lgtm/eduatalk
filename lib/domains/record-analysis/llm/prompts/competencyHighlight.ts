...
**issues 배열 형식 규칙 (엄수)**
- 패턴이 감지되면 반드시 위 코드(\`P1_나열식\`, \`F2_인과단절\`, \`F10_성장부재\` 등)를 **그대로** issues 배열에 넣으세요.
- "구체적 성과 부족", "탐구 깊이 부족" 같은 자연어 서술만 반환하는 것은 **금지**입니다. 자연어 서술이 필요하면 코드 **뒤에** 추가 요소로만 넣으세요.
- ❌ 나쁜 예: \`"issues": ["구체적 성과 부족", "탐구 깊이 부족"]\`  ← 코드 누락, 자연어만
- ✅ 좋은 예: \`"issues": ["P1_나열식", "F10_성장부재", "구체적 성과 부족"]\`  ← 코드 먼저, 자연어는 보조
- 해당 패턴이 없으면 \`issues\` 배열을 비워두거나 자연어만 포함해도 됩니다. 단, P-코드(구조적) / F-코드(과학적·거시) 중 하나라도 해당하면 **반드시** 코드부터 반환합니다.

**코드 매핑 힌트 (자연어 → 코드 변환)**
자연어 서술이 나올 때 아래 코드 중 해당되면 반드시 먼저 반환:
- "단원명·개념 나열", "배웠다 반복", "개념 정리만 함" → \`P1_나열식\`
- "전문용어는 있으나 탐구 내용 없음", "키워드 나열" → \`P3_키워드만\`
- "실험결과와 결론이 무관", "탐구 주제와 결론 사이 논리 비약", "A→B→C 3단 점프" → \`F2_인과단절\`
  - **구조적 탐지 cue (F2 필수)**: 교과 개념 학습 문장 뒤에 중간 연결고리 없이 "~되고 싶다 / ~되기로 결심했다 / ~가 되기로 하였다 / ~학자가 되겠다" 같은 **진로·사회이슈 결심 문장**이 바로 붙으면 F2 필수. 예: "뉴턴 법칙 → 블랙홀 → 환경 정책 사회학자", "미적분 → AI → 기본소득 복지정책학자", "공간벡터 → 건축 → 환경공학자". **이 패턴은 P1/P3/F12와 동시 감지되더라도 F2를 반드시 함께 반환**.
- "교사 과제 중심", "스스로 질문한 흔적 없음", "수동적 수용" → \`F12_자기주도성부재\`
- "모든 교과에 같은 진로 키워드", "과도한 진로 강조로 교과 역량 불명확" → \`F16_진로과잉도배\`
- "교사가 관찰할 수 없는 내면 상태 서술", "학생 자술만 있고 교사 평가 문장 전무" → \`M1_교사관찰불가\`
  - **M1 단독 탐지 규칙 (필수)**: 문장 전체가 "~했다 / ~되었다 / ~할 수 있다 / ~익혔다 / ~공부하였다" 등 학생 1인칭 자술로만 구성되고, "교사가 ~평가함 / ~설명함 / ~관찰됨 / 학생이 ~하는 모습을 보임" 류의 **교사 관찰·평가 서술이 0개**면 다른 패턴(P1/F12 등)과 별개로 **M1을 반드시 포함**. P1_나열식 / F12_자기주도성부재와 자주 동반되므로 함께 반환 가능.
- "학년 간 내용 깊이 동일", "성장 곡선 없음" (다학년 비교 가능할 때만) → \`F10_성장부재\`

**동반 패턴 매트릭스 (필수 동시 반환)**
한 가지 코드만 보고 멈추지 마세요. 아래 동반 패턴이 자주 함께 나타납니다 — 양쪽 모두 해당되면 **반드시 모두 포함**:

- **P1_나열식 + F12_자기주도성부재**: 개념을 단순 나열하면서 학생이 스스로 질문/탐구한 흔적이 없을 때. (예: "원소 주기율표를 공부했다. 전자배치를 배웠다. 문제를 풀었다.") → 둘 다 포함
- **P1_나열식 + P3_키워드만**: 전문용어를 나열하지만 탐구 과정 서술이 없을 때. → 둘 다 포함
- **P1_나열식 + F16_진로과잉도배**: 활동 나열에 진로 키워드가 강제 삽입되어 있을 때. → 둘 다 포함
- **P3_키워드만 + F2_인과단절**: 키워드는 있으나 결론이 탐구 주제와 단절되어 있을 때 (예: "유전법칙 → 자유의지 → 종교철학자" 3단 도약). → 둘 다 포함
- **F12_자기주도성부재 + F2_인과단절**: 교사 주도 과제에서 갑작스런 진로 결심으로 점프할 때. → 둘 다 포함
- **F16_진로과잉도배 + F12_자기주도성부재**: 교과와 무관한 진로 도배가 자기주도성 결여를 가릴 때. → 둘 다 포함
- **P1_나열식 + M1_교사관찰불가**: 개념 나열 + 교사 관찰 서술 전무 (학생 자술뿐). → 둘 다 포함
- **F12_자기주도성부재 + M1_교사관찰불가**: 수동적 학생 자술만 있고 교사 평가 문장이 없을 때. → 둘 다 포함
- **P3_키워드만 + F2_인과단절 + F16_진로과잉도배**: 교과 키워드 → 진로 결심 3단 점프에 진로 도배까지 겹칠 때 세 코드 모두 포함

**검수 체크리스트** (issues 배열 반환 직전):
1. 첫 번째로 떠오른 코드 외에 위 매트릭스 동반 패턴이 해당되는가?
2. F2/F16/M1처럼 모델이 자연어로 서술하기 쉬운 패턴을 코드로 변환했는가?
3. 코드 순서: critical > major > minor 순으로 정렬됐는가?
...
- **issues**: 발견된 품질 문제 목록. **위 패턴 코드(P1~F16, M1)가 해당되면 반드시 해당 코드 문자열을 먼저** 포함. 자유 기술(예: "동어반복")은 코드 **뒤에** 보조 요소로만 추가. 자연어만으로 반환하면 실패로 간주됩니다.
