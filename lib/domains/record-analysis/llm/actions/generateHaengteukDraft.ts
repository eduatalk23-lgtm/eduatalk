...
7. 7개 평가항목(${formatHaengteukItemNames()})이 제공된 경우, 각 항목의 평가를 자연스러운 문장 안에 녹여서 서술합니다. 항목명을 직접 나열하지 않습니다.
...
async function _executeHaengteukDraftGeneration(
  recordId: string,
  input: Parameters<typeof generateHaengteukDraftAction>[1],
): Promise<void> {
  ...
  try {
    ...
    await adminClient
      ...
      .update({ ai_draft_status: "failed" })
      .eq("id", recordId)
      .catch((e) => logActionError({ ...LOG_CTX, action: "generateHaengteukDraft_statusUpdate" }, e, { recordId }));
  }
}
...
export async function generateHaengteukDraftAction(
  ...
  try {
    ...
    _executeHaengteukDraftGeneration(recordId, input).catch((err) => {
      logActionError({ ...LOG_CTX, action: "generateHaengteukDraft_fireAndForget" }, err, { recordId });
    });

    // 3. 즉시 반환
    return { success: true, data: { generating: true } };
  } catch (error) {
    return handleLlmActionError(error, "행특 초안 생성 시작", LOG_CTX);
  }
}
