<!-- 99618d12-58c9-41d3-9ce8-e687b26e6c96 758550fa-ab54-4b24-9782-5d28cade3117 -->
# 마스터 콘텐츠 URL 필드 저장 수정 및 최적화 계획
...
**파일**: `lib/types/plan/domain.ts` (611-639줄)

**확인 사항**:

- `MasterLecture` 타입에 `lecture_source_url: string | null` 정의되어 있음 ✅
- `cover_image_url?: string | null` 정의되어 있지만 DB에 컬럼 없음

**조치**:

- 타입 정의는 유지 (향후 확장 가능성)
- 실제 저장 시에는 DB에 있는 필드만 사용

### 4. 폼 파싱 함수 확인

**파일**: `lib/utils/masterContentFormHelpers.ts` (220-246줄, 253-284줄)

**확인 결과**:

- `parseMasterLectureFormData`: `lecture_source_url` 파싱됨 ✅
- `parseMasterLectureUpdateFormData`: `lecture_source_url` 파싱됨 ✅

**조치**: 파싱 함수는 정상이므로 수정 불필요

## 구현 단계

### Step 1: `createMasterLecture` 수정

- [ ] `lecture_source_url` 필드를 insert 객체에 추가

### Step 2: `updateMasterLecture` 리팩토링

- [ ] `updateMasterBook`과 동일한 패턴으로 변경
- [ ] undefined 필드 제외 로직 적용
- [ ] `lecture_source_url` 필드 추가
...
- [ ] createMasterLecture 함수에 lecture_source_url 필드 추가
- [ ] updateMasterLecture 함수를 updateMasterBook과 동일한 패턴으로 리팩토링 (undefined 필드 제외, lecture_source_url 추가)
- [ ] TypeScript 타입 체크 및 ESLint 검증