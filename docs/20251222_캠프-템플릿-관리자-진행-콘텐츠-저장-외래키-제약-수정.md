...
#### 1. `continueCampStepsForAdmin` - `savePlanContents` 함수 사용 (Step 4-6)

**파일**: `app/(admin)/actions/camp-templates/progress.ts` (line 725-735)
...
#### 2. `continueCampStepsForAdmin` - `savePlanContents` 함수 사용 (Step 7)

**파일**: `app/(admin)/actions/camp-templates/progress.ts` (line 968-1020)

Step 7에서 플랜 생성 전 콘텐츠 저장 로직도 동일하게 수정했습니다.
...
#### 3. `validateAndResolveContent` - 복사 실패 시 콘텐츠 제외
...
#### 4. 플랜 생성 시 `contentIdMap`에 없는 콘텐츠 제외
...
플랜 생성 시 `contentIdMap`에 없는 콘텐츠를 제외하도록 수정했습니다.

**변경 전**:
```typescript
const plansForAssign = datePlans.map((plan, originalIndex) => {
  const finalContentId =
    contentIdMap.get(plan.content_id) || plan.content_id; // ❌ contentIdMap에 없으면 원본 ID 사용
  return {
