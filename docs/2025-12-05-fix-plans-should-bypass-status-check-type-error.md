...
## 원인 분석
`shouldBypassStatusCheck` 함수는 `planType: string | null` 타입을 받는데, `group.plan_type`은 `PlanType | null | undefined` 타입입니다. `undefined`를 처리하지 않아서 타입 에러가 발생했습니다.
...
#### 수정: group.plan_type에 nullish coalescing 연산자 적용
