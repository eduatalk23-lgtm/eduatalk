...
- `app/(admin)/admin/camp-templates/[id]/participants/[groupId]/review/page.tsx`
- `app/(admin)/admin/camp-templates/[id]/participants/[groupId]/review/CampPlanGroupReviewForm.tsx`
- `app/(admin)/admin/plan-groups/[id]/page.tsx` ⭐ **추가 수정**

### 4. 관리자 플랜 그룹 상세 페이지 수정

#### `app/(admin)/admin/plan-groups/[id]/page.tsx`

**문제점**:
- 잘못된 테이블 사용: `template_block_sets`, `template_blocks` 사용
- 블록 세트 ID 조회 우선순위가 다른 페이지와 다름
- `templateBlockSetId`와 `campTemplateId`를 `PlanGroupDetailView`에 전달하지 않음

**변경 사항**:
- 올바른 테이블 사용: `tenant_block_sets`, `tenant_blocks`로 변경
- 블록 세트 조회 우선순위를 다른 페이지들과 동일하게 변경
- `templateBlockSetId` 변수 추가 및 저장
- `PlanGroupDetailView`에 `templateBlockSetId`와 `campTemplateId` 전달

```typescript
let templateBlockSetId: string | null = null;

// 1. 연결 테이블에서 직접 조회 (1순위)
const { data: templateBlockSetLink } = await supabase
  .from("camp_template_block_sets")
  .select("tenant_block_set_id")
  .eq("camp_template_id", group.camp_template_id)
...
// 2. scheduler_options에서 확인 (2순위 - Fallback)
// 3. template_data에서 확인 (3순위 - 하위 호환성)

// 올바른 테이블 사용
const { data: templateBlockSet } = await supabase
  .from("tenant_block_sets")  // ✅ 올바른 테이블
  .select("id, name")
  .eq("id", blockSetId)
...
- 모든 영역(학생, 관리자)에서 동일한 우선순위 로직을 사용하여 일관성 유지
- **중요**: `template_block_sets`/`template_blocks`가 아닌 `tenant_block_sets`/`tenant_blocks` 테이블 사용
