# 캠프 모드 마스터 콘텐츠 ID 변환 추가

## 📋 작업 개요

캠프 모드에서 `student_plan`의 `content_id`가 마스터 콘텐츠 ID인 경우, 이를 학생 콘텐츠 ID로 변환하여 episodes를 정상적으로 조회할 수 있도록 수정했습니다.

## 🔍 문제 상황

### 근본 원인

`student_plan` 테이블의 `content_id`는 플랜 생성 시 저장된 값입니다. 플랜 생성 시 마스터 콘텐츠 ID가 그대로 저장될 수 있는데, 캠프 모드에서는 이를 변환하지 않고 그대로 사용하여 episodes 조회에 실패했습니다.

**일반 모드**:
- `resolveContentIds`를 통해 마스터 콘텐츠 ID를 학생 콘텐츠 ID로 변환
- 변환된 학생 콘텐츠 ID를 사용하여 episodes 조회

**캠프 모드 (수정 전)**:
- `student_plan`에서 `content_id`를 직접 읽어옴
- 마스터 콘텐츠 ID인 경우 변환하지 않음
- `getStudentLectureEpisodesBatch`에 마스터 콘텐츠 ID를 전달
- `lectures` 테이블에서 해당 ID로 학생 강의를 찾지 못함

### 문제 발생 시나리오

1. 플랜 생성 시 `student_plan.content_id`에 마스터 콘텐츠 ID가 저장됨
2. 캠프 모드에서 `getCampDatePlans` 호출
3. `student_plan`에서 `content_id`를 읽어옴 (마스터 콘텐츠 ID)
4. `getStudentLectureEpisodesBatch(studentLectureIds, studentId)` 호출
5. `lectures` 테이블에서 `id = content_id AND student_id = studentId`로 조회
6. 마스터 콘텐츠 ID이므로 조회 실패
7. Fallback 로직이 작동하지만, `lectures` 테이블에서 `master_lecture_id`를 찾을 때도 실패할 수 있음

## ✅ 해결 방법

### 핵심 아이디어

`student_plan`의 `content_id`가 마스터 콘텐츠 ID인 경우, 해당 학생의 학생 콘텐츠 ID로 변환하여 episodes를 조회합니다.

### 수정 내용

#### 파일: `lib/data/campLearning.ts`

**1. content_id 변환 맵 생성**

```typescript
// content_id가 마스터 콘텐츠 ID일 수 있으므로 변환 맵 생성
const contentIdMap = new Map<string, string>(); // originalContentId -> studentContentId
```

**2. 마스터 콘텐츠 ID 확인 및 변환**

```typescript
if (lectureIds.length > 0) {
  // 1차: student_plan의 content_id로 직접 조회 (학생 콘텐츠 ID인 경우)
  const { data: lectures } = await supabase
    .from("lectures")
    .select("id, title, subject, master_content_id, master_lecture_id")
    .in("id", lectureIds);
  
  // 조회된 학생 강의들 매핑
  lectures?.forEach((lecture) => {
    contentIdMap.set(lecture.id, lecture.id); // 자기 자신으로 매핑
  });
  
  // 2차: 조회되지 않은 content_id들은 마스터 콘텐츠 ID일 수 있음
  const notFoundLectureIds = lectureIds.filter((id) => !foundLectureIds.has(id));
  
  if (notFoundLectureIds.length > 0) {
    // 마스터 강의인지 확인
    const { data: masterLectures } = await supabase
      .from("master_lectures")
      .select("id, title, subject")
      .in("id", notFoundLectureIds);
    
    if (masterLectures && masterLectures.length > 0) {
      // 각 학생별로 마스터 강의 ID에 해당하는 학생 강의 찾기
      for (const [studentId, studentLectureIds] of studentLectureMap.entries()) {
        const masterIdsInStudent = studentLectureIds.filter((id) =>
          notFoundLectureIds.includes(id)
        );
        
        if (masterIdsInStudent.length > 0) {
          // 해당 학생의 강의 중 master_content_id 또는 master_lecture_id가 일치하는 것 찾기
          const { data: studentLecturesByMaster } = await supabase
            .from("lectures")
            .select("id, master_content_id, master_lecture_id")
            .eq("student_id", studentId)
            .or(
              `master_content_id.in.(${masterIdsInStudent.join(",")}),master_lecture_id.in.(${masterIdsInStudent.join(",")})`
            );
          
          // 마스터 ID -> 학생 ID 매핑 생성
          studentLecturesByMaster?.forEach((studentLecture) => {
            const masterId =
              studentLecture.master_content_id || studentLecture.master_lecture_id;
            if (masterId && masterIdsInStudent.includes(masterId)) {
              contentIdMap.set(masterId, studentLecture.id);
            }
          });
        }
      }
    }
  }
}
```

**3. 변환된 ID로 episodes 조회**

```typescript
for (const [studentId, studentLectureIds] of studentLectureMap.entries()) {
  if (studentLectureIds.length > 0) {
    // content_id가 마스터 콘텐츠 ID인 경우 학생 콘텐츠 ID로 변환
    const resolvedLectureIds = studentLectureIds.map(
      (id) => contentIdMap.get(id) || id
    );
    const episodes = await getStudentLectureEpisodesBatch(resolvedLectureIds, studentId);
    
    // 원본 content_id로 매핑하여 저장 (formatLectureRange에서 원본 content_id 사용)
    const episodesWithOriginalId = new Map<string, Array<{...}>>();
    
    studentLectureIds.forEach((originalId, index) => {
      const resolvedId = resolvedLectureIds[index];
      const episodesForResolvedId = episodes.get(resolvedId) || [];
      episodesWithOriginalId.set(originalId, episodesForResolvedId);
    });
    
    lectureEpisodesMap.set(studentId, episodesWithOriginalId);
  }
}
```

## 🔄 동작 흐름

```
1. student_plan에서 content_id 읽기
   ↓
2. lectures 테이블에서 직접 조회 (학생 콘텐츠 ID인 경우)
   ↓
3. 조회되지 않은 content_id는 마스터 콘텐츠 ID일 수 있음
   ↓
4. master_lectures 테이블에서 확인
   ↓
5. 각 학생별로 master_content_id 또는 master_lecture_id로 학생 강의 찾기
   ↓
6. contentIdMap에 마스터 ID -> 학생 ID 매핑 저장
   ↓
7. 변환된 학생 콘텐츠 ID로 episodes 조회
   ↓
8. 원본 content_id로 매핑하여 저장
```

## 🎯 기대 효과

1. **정상적인 Episodes 조회**: 마스터 콘텐츠 ID가 저장된 경우에도 정상적으로 episodes 조회 가능
2. **일반 모드와 일관성**: 일반 모드와 동일한 방식으로 content_id 변환
3. **Fallback 로직 보완**: `getStudentLectureEpisodesBatch`의 fallback 로직과 함께 작동하여 더 안정적인 조회

## 🔗 관련 코드

- `lib/data/campLearning.ts`: `getCampDatePlans` 함수
- `lib/plan/contentResolver.ts`: `resolveContentIds` 함수 (일반 모드 참고)
- `lib/data/contentMasters.ts`: `getStudentLectureEpisodesBatch` 함수 (fallback 로직)

## 📅 작업 일시

2025-12-22 13:38:56


