...
- [x] `bulkCreatePlanGroupsForCamp` 함수에서 템플릿 블록 세트 ID 조회 로직 추가
- [x] `getCampPlanGroupForReview` 함수의 블록 세트 조회 로직 참고하여 구현
- [x] 템플릿 블록 세트 ID를 `creationData.block_set_id`에 설정
...
- [x] 보호 필드 목록 정의 (`template_block_set_id` 등)
- [x] 병합 전 보호 필드 제외 로직 구현
- [x] `_createPlanGroup` 함수 수정
- [x] `_savePlanGroupDraft` 함수에도 동일 로직 적용
- [x] 기존 복원 로직 제거 (사전 방지로 대체)
- [ ] 단위 테스트 작성
...
- [x] `generateTimeSlots` 함수가 `daily_schedule`에 포함되는지 확인 (이미 포함됨 확인)
- [x] `calculateAvailableTimeForDate` 함수의 반환값에 `time_slots` 포함 여부 확인 (이미 포함됨 확인)
- [x] `daily_schedule` 생성 시 `time_slots`가 항상 포함되도록 보장 (이미 보장됨)
- [x] 저장 전 `time_slots` 검증 로직 추가
- [x] 플랜 그룹 생성 시 `daily_schedule`의 `time_slots` 확인 로직 추가
- [ ] 통합 테스트 작성
...
const nonStudyTimeBlocksSchema = z.array(nonStudyTimeBlockSchema).refine(
  (blocks) => {
    // 중복 체크
    const keys = new Set();
    for (const block of blocks) {
      const key = `${block.start_time}-${block.end_time}-${
        block.day_of_week?.join(",") || "all"
      }`;
...
- [x] 1.1 캠프 모드 `block_set_id` null 처리 수정 (완료: 2025-02-01)
- [x] 1.2 `time_settings` 병합 시 보호 필드 명시적 처리 (완료: 2025-02-01)
- [x] 1.3 `daily_schedule` 생성 및 저장 흐름 명확화 (완료: 2025-02-01)
