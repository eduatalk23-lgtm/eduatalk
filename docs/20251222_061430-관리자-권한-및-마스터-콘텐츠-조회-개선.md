# 관리자 권한 및 마스터 콘텐츠 조회 개선

## 📋 작업 개요

관리자 영역에서 학생 권한 확인 부분 개선 및 마스터 콘텐츠 조회 로직 개선 작업을 수행했습니다.

## 🔍 문제 상황

### 1. 관리자 권한 오류

관리자가 학생 대신 플랜 그룹을 업데이트할 때 "학생 권한이 필요합니다"라는 오류가 발생했습니다.

**에러 스택**:
```
Error [AppError]: 학생 권한이 필요합니다.
  at requireStudentAuth (lib/auth/requireStudentAuth.ts:34:11)
  at async _updatePlanGroupDraft (app/(student)/actions/plan-groups/update.ts:33:16)
```

**원인**:
- `_updatePlanGroupDraft` 함수가 `requireStudentAuth()`만 호출하여 학생 권한만 허용
- 관리자 모드에서도 `PlanGroupWizard`를 사용하여 `updatePlanGroupDraftAction`을 호출
- 관리자 모드에서는 특정 학생을 대신하여 플랜 그룹을 수정해야 하므로 `student_id`가 필요

### 2. 마스터 콘텐츠 조회 누락

마스터 커스텀 콘텐츠가 조회되지 않는 문제가 있었습니다.

**원인**:
- `classifyPlanContents` 함수에서 `custom` 타입의 콘텐츠는 `student_custom_contents`에서만 조회
- `master_custom_contents` 테이블에서 조회하지 않음
- `plan_contents.master_content_id`가 있어도 마스터 커스텀 콘텐츠를 조회하지 않음

### 3. 블록 세트 조회 문제

제출한 내용 확인 페이지에서 블록 세트가 조회되지 않는 문제가 있었습니다.

**원인 분석**:
- 제출한 내용 확인 페이지에서는 `parseCampConfiguration` 함수를 사용하여 블록 세트를 조회
- 이 함수는 `resolveCampBlockSetId`를 사용하여 블록 세트 ID를 조회
- 조회 우선순위:
  1. 연결 테이블(`camp_template_block_sets`)에서 직접 조회
  2. `scheduler_options.template_block_set_id` 확인
  3. `template_data.block_set_id` 확인
- `campActions.ts`에서 제출 시 `template_block_set_id`를 `scheduler_options`에 저장하고 있음
- `getPlanGroupWithDetails`에서 `scheduler_options`를 조회하고 있음

## 🛠 해결 방법

### 1. `_updatePlanGroupDraft` 함수 수정

**파일**: `app/(student)/actions/plan-groups/update.ts`

관리자/컨설턴트 권한도 허용하도록 수정:

```typescript
async function _updatePlanGroupDraft(
  groupId: string,
  data: Partial<PlanGroupCreationData>
): Promise<void> {
  // 권한 확인: 학생 또는 관리자/컨설턴트
  const currentUser = await getCurrentUser();
  if (!currentUser) {
    throw new AppError(
      "로그인이 필요합니다.",
      ErrorCode.UNAUTHORIZED,
      401,
      true
    );
  }

  const isAdmin = currentUser.role === "admin" || currentUser.role === "consultant";
  let studentId: string;
  let userId: string;

  if (isAdmin) {
    // 관리자 모드: 권한 확인 및 플랜 그룹에서 student_id 조회
    await requireAdminOrConsultant();
    userId = currentUser.userId;

    // 플랜 그룹 조회하여 student_id 확인
    const tenantContext = await requireTenantContext();
    const result = await getPlanGroupWithDetailsForAdmin(
      groupId,
      tenantContext.tenantId
    );

    if (!result.group) {
      throw new AppError(
        "플랜 그룹을 찾을 수 없습니다.",
        ErrorCode.NOT_FOUND,
        404,
        true
      );
    }

    studentId = result.group.student_id;
  } else {
    // 학생 모드: 현재 사용자가 학생
    const studentAuth = await requireStudentAuth();
    studentId = studentAuth.userId;
    userId = studentAuth.userId;
  }

  // ... 나머지 로직
}
```

**주요 변경사항**:
- `getCurrentUser()`로 현재 사용자 정보 조회
- 관리자/컨설턴트 권한 확인 추가
- 관리자 모드일 때 `getPlanGroupWithDetailsForAdmin`로 플랜 그룹 조회하여 `student_id` 확인
- 학생 모드일 때는 기존처럼 `requireStudentAuth()` 사용
- `updatePlanGroup` 호출 시 `studentId` 사용 (관리자 모드에서도 올바른 학생 ID 사용)

### 2. 마스터 커스텀 콘텐츠 조회 추가

**파일**: `lib/data/planContents.ts`

마스터 커스텀 콘텐츠 조회 로직 추가:

```typescript
// 1. 마스터 커스텀 콘텐츠 ID 수집
const masterCustomContentIds: string[] = [];

contents.forEach((content) => {
  if (content.content_type === "custom") {
    customContentIds.push(content.content_id);
    // plan_contents에 저장된 master_content_id가 있으면 마스터 커스텀 콘텐츠 ID로도 수집
    if (content.master_content_id) {
      masterCustomContentIds.push(content.master_content_id);
    }
    // content_id 자체가 마스터 커스텀 콘텐츠 ID일 수 있으므로 마스터 콘텐츠 조회 대상에 포함
    masterCustomContentIds.push(content.content_id);
  }
});

// 2. 마스터 커스텀 콘텐츠 조회
const masterCustomContentsResult = uniqueMasterCustomContentIds.length > 0
  ? supabase
      .from("master_custom_contents")
      .select("id, title, subject_category, subject")
      .in("id", uniqueMasterCustomContentIds)
  : Promise.resolve({ data: [], error: null });

// 3. Map으로 변환
const masterCustomContentsMap = new Map(
  (masterCustomContentsResult.data || []).map((custom) => [custom.id, custom])
);

// 4. custom 타입 콘텐츠 처리 로직 개선
else if (content.content_type === "custom") {
  // 1. plan_contents에 저장된 master_content_id가 있으면 우선 활용
  const masterCustomContentFromPlan = content.master_content_id
    ? masterCustomContentsMap.get(content.master_content_id)
    : null;

  // 2. content_id로 학생 커스텀 콘텐츠 조회
  const customContent = customContentsMap.get(content.content_id);

  if (customContent) {
    // 학생 커스텀 콘텐츠를 찾은 경우
    contentDetail = {
      content_type: "custom",
      content_id: content.content_id,
      start_range: content.start_range,
      end_range: content.end_range,
      title: customContent.title || "커스텀 콘텐츠",
      subject_category: customContent.content_type || null,
      isRecommended: false,
    };
  } else if (masterCustomContentFromPlan) {
    // 학생 커스텀 콘텐츠를 찾지 못했지만 plan_contents에 master_content_id가 있는 경우
    // → 마스터 커스텀 콘텐츠 (추천 콘텐츠)
    contentDetail = {
      content_type: "custom",
      content_id: content.content_id,
      start_range: content.start_range,
      end_range: content.end_range,
      title: masterCustomContentFromPlan.title || "커스텀 콘텐츠",
      subject_category: masterCustomContentFromPlan.subject_category || masterCustomContentFromPlan.subject || null,
      isRecommended: true, // 마스터 콘텐츠이므로 추천 콘텐츠
      masterContentId: content.master_content_id ?? undefined,
      // 자동 추천 정보 전달
      is_auto_recommended: content.is_auto_recommended ?? false,
      recommendation_source: content.recommendation_source ?? null,
      recommendation_reason: content.recommendation_reason ?? null,
      recommendation_metadata: content.recommendation_metadata ?? null,
    };
  } else {
    // content_id 자체가 마스터 커스텀 콘텐츠 ID인지 확인
    const masterCustomContentByContentId = masterCustomContentsMap.get(content.content_id);
    if (masterCustomContentByContentId) {
      // content_id가 마스터 커스텀 콘텐츠 ID인 경우 → 추천 콘텐츠
      contentDetail = {
        content_type: "custom",
        content_id: content.content_id,
        start_range: content.start_range,
        end_range: content.end_range,
        title: masterCustomContentByContentId.title || "커스텀 콘텐츠",
        subject_category: masterCustomContentByContentId.subject_category || masterCustomContentByContentId.subject || null,
        isRecommended: true,
        masterContentId: content.content_id,
        // 자동 추천 정보 전달
        is_auto_recommended: content.is_auto_recommended ?? false,
        recommendation_source: content.recommendation_source ?? null,
        recommendation_reason: content.recommendation_reason ?? null,
        recommendation_metadata: content.recommendation_metadata ?? null,
      };
    } else {
      // 정말로 찾을 수 없는 경우
      missingContents.push({
        content_type: "custom",
        content_id: content.content_id,
        reason: `학생(${studentId})의 커스텀 콘텐츠를 찾을 수 없습니다. master_custom_contents에도 존재하지 않습니다.`,
      });
    }
  }
}
```

**주요 변경사항**:
- `masterCustomContentIds` 배열 추가하여 마스터 커스텀 콘텐츠 ID 수집
- `master_custom_contents` 테이블에서 조회 추가
- `masterCustomContentsMap` 생성하여 빠른 조회 지원
- `custom` 타입 콘텐츠 처리 로직 개선:
  - 학생 커스텀 콘텐츠 우선 조회
  - `plan_contents.master_content_id`로 마스터 커스텀 콘텐츠 조회
  - `content_id` 자체가 마스터 커스텀 콘텐츠 ID인지 확인
  - 마스터 커스텀 콘텐츠는 추천 콘텐츠로 분류

### 3. 블록 세트 조회 로직 확인

**파일**: `app/(student)/camp/[invitationId]/submitted/page.tsx`

제출한 내용 확인 페이지에서 블록 세트 조회 로직을 확인했습니다.

**현재 로직**:
1. `getPlanGroupWithDetails`로 플랜 그룹 조회
2. `scheduler_options`가 `undefined`인 경우 직접 조회 시도
3. `parseCampConfiguration` 함수로 블록 세트 조회
4. `resolveCampBlockSetId` 함수로 블록 세트 ID 조회 (3단계 우선순위)
5. `fetchCampTemplateBlocks` 함수로 블록 목록 조회

**확인 사항**:
- `getPlanGroupById`는 `scheduler_options`를 조회하고 있음
- `campActions.ts`에서 제출 시 `template_block_set_id`를 `scheduler_options`에 저장하고 있음
- `parseCampConfiguration` 함수가 올바르게 작동하고 있음

**결론**:
블록 세트 조회 로직은 정상적으로 작동하고 있습니다. 만약 블록 세트가 표시되지 않는다면:
1. `scheduler_options.template_block_set_id`가 저장되었는지 확인
2. `camp_template_block_sets` 연결 테이블에 데이터가 있는지 확인
3. `tenant_block_sets`와 `tenant_blocks` 테이블에 데이터가 있는지 확인

## 📝 변경된 파일

1. `app/(student)/actions/plan-groups/update.ts`
   - `_updatePlanGroupDraft` 함수에 관리자 권한 지원 추가

2. `lib/data/planContents.ts`
   - 마스터 커스텀 콘텐츠 조회 로직 추가
   - `custom` 타입 콘텐츠 처리 로직 개선

## ✅ 검증 체크리스트

- [x] 관리자 모드에서 플랜 그룹 업데이트 가능
- [x] 학생 모드에서 플랜 그룹 업데이트 가능
- [x] 마스터 커스텀 콘텐츠 조회 가능
- [x] 학생 커스텀 콘텐츠 조회 가능
- [x] 마스터 커스텀 콘텐츠가 추천 콘텐츠로 분류됨
- [x] 블록 세트 조회 로직 확인 완료

## 🔗 관련 파일

- `app/(student)/actions/plan-groups/update.ts` - 플랜 그룹 업데이트 액션
- `lib/data/planContents.ts` - 콘텐츠 분류 및 조회 로직
- `app/(student)/camp/[invitationId]/submitted/page.tsx` - 제출한 내용 확인 페이지
- `lib/camp/campAdapter.ts` - 캠프 설정 파싱 로직
- `app/(student)/actions/campActions.ts` - 캠프 참여 정보 제출 액션

## 📝 참고 사항

- 관리자 모드에서 플랜 그룹을 업데이트할 때는 `getPlanGroupWithDetailsForAdmin`를 사용하여 `student_id`를 조회합니다.
- 마스터 커스텀 콘텐츠는 `master_custom_contents` 테이블에서 조회하며, 추천 콘텐츠로 분류됩니다.
- 블록 세트 조회는 `parseCampConfiguration` 함수를 사용하며, 3단계 우선순위로 조회합니다.

## 날짜

2025-12-22



