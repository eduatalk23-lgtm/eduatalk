...
```typescript
const debouncedCalculate = useMemo(() => debounce(calculateSchedule, 500), []);
```
...
const validateStep = (step: 1 | 2 | 3 | 4 | 5) => {
  switch (step) {
    ...
    case 2:
      return validateTimeSettingsAndSchedule(data); // 통합
    case 3:
      return validateContents(data); // 통합
    case 4:
      return validateFinalReview(data);
    case 5:
      return true; // 완료는 항상 통과
  }
};
```
