...
1. **Phase 1**: 하드코딩된 `subject_id` 제거, 실제 데이터베이스 `subject_id` 사용
2. **Phase 2**: 통합 컴포넌트 생성 및 기존 컴포넌트 교체
3. **Phase 3**: 검증 로직 개선 및 초기화 로직 개선
4. **Phase 4**: 문서화 완료

### 주요 개선 사항

- **코드 중복 제거**: 3개 컴포넌트 → 1개 통합 컴포넌트
- **일관된 UI 패턴**: 교재/강의 콘텐츠 선택 UI와 일관된 패턴
- **정확성 향상**: 실제 `subject_id` 사용으로 정확한 매칭
- **유지보수성 향상**: 공통 컴포넌트 재사용
