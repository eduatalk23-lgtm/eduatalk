...
### 10.8 슬롯 관계 시각적 피드백 (구현 완료)

**SlotItem 관계 배지**:
- 연계 슬롯: 🔗 아이콘 + 연결된 슬롯 이름 + 방향 표시 (← after, → before)
- 배타적 슬롯: ≠ 아이콘 + 슬롯 수 또는 이름

**SlotAdvancedSettings UI 개선**:
- "슬롯 연결 (순서 강제)" - 레이블 명확화
- "다른 날 배치 (배타적)" - 새 섹션 추가
  - 현재 배타적 관계: 태그로 표시 (X 버튼으로 제거)
  - 슬롯 추가: 드롭다운 선택

### 10.9 데이터 무결성 및 안정성 (구현 완료)

#### Draft 플랜 레이스 컨디션 방지

**문제**: 동시에 두 요청이 draft 플랜 그룹을 생성할 때 중복 생성 발생

**해결**:
1. **Partial Unique Index** (Supabase 마이그레이션)
```sql
-- 동일 학생의 동일 이름 draft는 1개만 허용
CREATE UNIQUE INDEX idx_plan_groups_unique_draft_with_name
ON plan_groups (student_id, name)
WHERE status = 'draft' AND deleted_at IS NULL AND name IS NOT NULL;

-- 이름 없는 draft도 1개만 허용
CREATE UNIQUE INDEX idx_plan_groups_unique_draft_unnamed
ON plan_groups (student_id)
WHERE status = 'draft' AND deleted_at IS NULL AND name IS NULL AND camp_invitation_id IS NULL;
```

2. **Unique Violation Retry Logic** (`create.ts`)
```typescript
if (groupResult.errorCode === "23505") {
  // 중복 발생 시 기존 draft 찾아서 업데이트
...
**파일**: `lib/domains/plan/repository.ts`

**함수**: `checkPlanPeriodOverlap(studentId, periodStart, periodEnd, excludeGroupId?)`

**동작**:
- 동일 학생의 활성/진행 중인 플랜과 기간 겹침 검사
- 겹침 조건: `new_start <= existing_end AND new_end >= existing_start`
- 생성 전 검증하여 중복 기간 방지
...
**함수**: `rollbackPlanGroupCreation(groupId, studentId)`

**동작**:
- 플랜 콘텐츠, 제외일, 학원 일정 병렬 삭제
- 플랜 그룹 삭제
- 생성 중 오류 시 자동 롤백
...
**파일**: `__tests__/content-selection/slotRelationships.test.ts`

**테스트 항목** (19개):
- 연계 슬롯 스케줄링 (같은 날 배치, 그룹 ID, 시간 부족 경고)
- 배타적 슬롯 스케줄링 (다른 날 배치, 양방향 관계, 경고)
- 복합 관계 (연계 + 배타 동시)
- 관계 검증 (자기 참조, 순환 참조, 존재하지 않는 슬롯)

#### 전체 테스트 현황

| 영역 | 테스트 수 | 파일 |
|------|----------|------|
| 슬롯 구성 | 39 | `slotConfiguration.test.ts` |
| 슬롯 관계 | 19 | `slotRelationships.test.ts` |
| 자동 매칭 | 28 | `autoMatchSlots.test.ts` |
| 가상 타임라인 | 24 | `virtualTimelineScheduling.test.ts` |
| Dual Write | 15 | `dualWriteConversion.test.ts` |
...
| P1 | 슬롯 관계 테스트 | ✅ 완료 |
| P1 | 플랜 기간 중복 검증 | ✅ 완료 |
| P1 | Draft 레이스 컨디션 방지 | ✅ 완료 |
| P1 | 에러 처리 일관성 | ✅ 완료 |
| P2 | 슬롯 관계 시각적 피드백 | ✅ 완료 |
| P2 | AI 기반 슬롯 추천 | ✅ 완료 |
| P2 | console.log 정리 (plan-groups 관련) | ✅ 완료 |
...
| v2.6 | 2025-12-23 | 슬롯 관계 시각적 피드백 (관계 배지, 배타적 슬롯 UI) |
| v2.7 | 2025-12-24 | 데이터 무결성 및 안정성 (Draft 레이스 컨디션, 기간 중복 검증, 롤백 로직) |
| v2.8 | 2025-12-24 | 테스트 커버리지 강화 (슬롯 관계 테스트 19개, 에러 처리 일관성) |
