...
### 문제점
로그에서 `count: null`이 확인되었습니다. UPDATE 쿼리에서 `.select("*", { count: "exact", head: true })`를 사용하면 RLS 정책으로 인해 SELECT가 실패하고, 이 경우 `count`가 `null`이 됩니다.

### 해결 방법
UPDATE 쿼리에서 `.select()`를 완전히 제거하고, UPDATE 후 별도로 상태를 확인하도록 수정했습니다.
...
const { error, count } = await supabase
  ...
  .eq("id", invitationId)
  .eq("status", "pending")
  .select("*", { count: "exact", head: true }); // RLS 정책으로 인해 count가 null이 될 수 있음
...
### 문제점
데이터베이스에 UPDATE 정책이 없었습니다. 마이그레이션 파일은 있었지만 실제로 적용되지 않았습니다.

### 해결 방법
마이그레이션을 다시 실행하여 UPDATE 정책을 생성했습니다.

**생성된 정책**:
- `camp_invitations_update_for_student`: 학생이 자신의 초대 상태를 업데이트할 수 있도록 허용
- `camp_invitations_update_for_admin`: 관리자/컨설턴트가 자신의 테넌트에 속한 초대를 업데이트할 수 있도록 허용

**정책 조건**:
- USING 절: `status = 'pending'` AND `student_id = auth.uid()`
- WITH CHECK 절: `status IN ('accepted', 'declined')` AND `student_id = auth.uid()`

이제 UPDATE가 정상적으로 작동해야 합니다.
