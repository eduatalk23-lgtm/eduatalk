...
### Phase 1: any 타입 제거
- **완료**: 29개 `any` 타입 제거
- **문서**: `docs/2025-02-04-repomix-phase2-improvements.md`

### Phase 2: Deprecated 함수 정리
- **완료**: 
  - `selectClientForStudentQuery` 등 4개 함수 → `selectClientForCrossAccess` 마이그레이션
  - `parseForm*` 함수들 제거
  - `isViewNotFoundError` 등 에러 체커 함수들 제거
- **문서**: `docs/2025-02-04-deprecated-functions-cleanup.md`
...
### Option 1: 레거시 student_scores 테이블 마이그레이션 (우선순위: 높음)

#### 개요
레거시 `student_scores` 테이블을 새로운 구조로 마이그레이션:
- 내신 성적: `student_internal_scores` 테이블
- 모의고사 성적: `student_mock_scores` 테이블

#### 발견된 Deprecated 코드
1. **`app/actions/scores.ts`**
   - `addStudentScore`, `updateStudentScore`, `deleteStudentScore` 함수
   - 레거시 `student_scores` 테이블 사용
   - 새 구조: `createInternalScore`, `createMockScore` 사용

2. **`app/(student)/scores/dashboard/_utils.ts`**
   - 레거시 `student_scores` 테이블 사용
   - 새 구조로 마이그레이션 필요

3. **`lib/data/studentScores.ts`**
   - `getStudentScores` 함수 deprecated
   - 새 구조: `getInternalScores`, `getMockScores` 사용

4. **`lib/reports/monthly.ts`**
   - `getMonthlyWeakSubjectTrend`에서 `student_scores` 참조
   - TODO 주석: `student_internal_scores`와 `student_mock_scores` 조합 필요

#### 작업 계획
1. **사용처 확인**
   - `app/actions/scores.ts`의 함수들이 실제로 사용되는지 확인
   - 사용 중이면 새 구조로 마이그레이션
   - 사용하지 않으면 제거

2. **마이그레이션**
   - `addStudentScore` → `createInternalScore` 또는 `createMockScore`
   - `updateStudentScore` → `updateInternalScore` 또는 `updateMockScore`
   - `deleteStudentScore` → `deleteInternalScore` 또는 `deleteMockScore`
...
### 1단계: 레거시 student_scores 마이그레이션 (Option 1)
**이유**: 
- 데이터 일관성에 중요한 부분
- 새로운 구조로 완전히 전환 필요
- 사용자 기능에 직접 영향
...
|------|---------|----------|--------|--------|
| Option 1: student_scores 마이그레이션 | 높음 | 2-3시간 | 높음 | 중간 |
