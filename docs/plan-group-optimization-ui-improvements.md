...
## ✨ 기능 추가: 플랜 목록 상세보기 스케줄 미리보기에 자율학습 시간 추가

### 변경 사항

플랜 목록의 상세보기 페이지에서 스케줄 미리보기 탭에 자율학습 시간 정보를 추가했습니다.

#### 1. 요약 정보에 자율학습 시간 카드 추가

- 요약 정보 섹션에 "자율학습 시간" 카드를 추가했습니다.
- `summary.total_self_study_hours`가 있으면 사용하고, 없으면 `daily_schedule`에서 계산합니다.
- 자율학습 시간이 0보다 큰 경우에만 표시됩니다.

#### 2. 주차별 스케줄에 자율학습 시간 표시

- 각 주차별로 자율학습 시간을 계산하여 표시합니다.
- 주차 헤더에 "자율학습 X시간" 형식으로 표시됩니다.
- 지정휴일의 경우 `study_hours`를 자율학습 시간으로 계산하고, 일반 학습일/복습일의 경우 `time_slots`에서 자율학습 시간을 계산합니다.

#### 3. timeSettings 전달

- `group.scheduler_options`에서 `time_settings`를 추출하여 `calculateScheduleAvailability`에 전달합니다.
- 이를 통해 정확한 자율학습 시간이 계산됩니다.
...
이제 플랜 목록의 상세보기에서 스케줄 미리보기 탭을 확인하면 요약 정보와 주차별 스케줄에 자율학습 시간이 표시됩니다.
