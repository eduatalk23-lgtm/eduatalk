...
- [ ] **[P1-1]** `student_plan` RLS 정책 마이그레이션 작성
  - 파일: `supabase/migrations/20251209000001_add_student_plan_rls.sql`
  - 학생 정책: `student_id = auth.uid()`
  - 관리자 정책: 같은 `tenant_id` 내 접근
  - 위험도: 🔴 높음 (기존 쿼리 영향)

- [ ] **[P1-2]** `updated_at` 자동 업데이트 트리거 작성
  - 파일: `supabase/migrations/20251209000002_add_updated_at_trigger.sql`
...
- [ ] **[P1-3]** 중앙 상수 파일 생성
  ...
  - 내용:
    ```typescript
    export const DUMMY_NON_LEARNING_CONTENT_ID = "00000000-0000-0000-0000-000000000000";
    export const DUMMY_SELF_STUDY_CONTENT_ID = "00000000-0000-0000-0000-000000000001";
    ```
  - 위험도: 🟢 낮음

- [ ] **[P1-4]** `isDummyContent()` 헬퍼 함수 생성
  - 파일: `lib/utils/planUtils.ts` (신규 또는 기존 파일)
  - 위험도: 🟢 낮음

- [ ] **[P1-5]** 기존 하드코딩 제거 및 import 교체
  - 대상 파일:
    - `lib/plan/generators/planDataPreparer.ts`
    - `app/(student)/actions/plan-groups/plans.ts`
  - 위험도: 🟡 중간

- [ ] **[P1-6]** 더미 콘텐츠 DB row 보장 시드 스크립트
  - 파일: `scripts/seed-dummy-contents.ts`
  - 위험도: 🟢 낮음
...
- [ ] **[P1-7]** 완료 기준 상수 정의
  ...
  - 내용:
    ...
    export const PLAN_COMPLETION_CRITERIA = {
      // 권장: actual_end_time 기준
      isCompleted: (plan: { actual_end_time: string | null }) => 
        plan.actual_end_time !== null
    };
    ```
  - 위험도: 🟢 낮음

- [ ] **[P1-8]** `isCompletedPlan()` 헬퍼 함수 생성
  - 파일: `lib/utils/planUtils.ts`
  - 위험도: 🟢 낮음

- [ ] **[P1-9]** `todayProgress.ts` 수정
  - 파일: `lib/metrics/todayProgress.ts`
  - 변경: `isCompletedPlan()` 헬퍼 사용
  - 위험도: 🔴 높음 (통계 변동 가능)

- [ ] **[P1-10]** `getPlanCompletion.ts` 수정
  - 파일: `lib/metrics/getPlanCompletion.ts`
  - 변경: `isCompletedPlan()` 헬퍼 사용
  - 위험도: 🔴 높음 (통계 변동 가능)

- [ ] **[P1-11]** 비학습/자율학습 집계 정책 문서화
