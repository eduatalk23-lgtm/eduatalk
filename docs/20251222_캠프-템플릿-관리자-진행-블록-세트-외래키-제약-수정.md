# 캠프 템플릿 관리자 진행 시 블록 세트 외래 키 제약 조건 수정

**작업 일시**: 2024년 12월 22일  
**작업자**: AI Assistant

## 📋 문제 상황

관리자가 캠프 템플릿 참여자의 남은 단계를 진행할 때(`continueCampStepsForAdmin`) 외래 키 제약 조건 위반 에러가 발생했습니다.

### 에러 메시지

```
Error [AppError]: 플랜 그룹 업데이트 실패: insert or update on table "plan_groups" violates foreign key constraint "plan_groups_block_set_id_fkey"
```

### 원인 분석

1. **잘못된 `block_set_id` 설정**
   - `continueCampStepsForAdmin` 함수에서 템플릿 블록 세트 ID(`tenantBlockSetId`)를 `creationData.block_set_id`에 직접 설정
   - `plan_groups.block_set_id`는 `student_block_sets` 테이블을 참조하는 외래 키
   - 템플릿 블록 세트는 `tenant_block_sets` 테이블에 있으므로 외래 키 제약 조건 위반 발생

2. **캠프 모드 처리 누락**
   - `campActions.ts`의 `submitCampParticipation`에서는 올바르게 처리하고 있음
   - `block_set_id = null`로 설정하고 `scheduler_options.template_block_set_id`에 저장
   - `continueCampStepsForAdmin`에서는 이 로직이 누락됨

## 🛠 해결 방법

### 수정 내용

**파일**: `app/(admin)/actions/camp-templates/progress.ts`

**변경 전**:
```typescript
creationData.plan_type = "camp";
if (result.group.camp_template_id) {
  creationData.camp_template_id = result.group.camp_template_id;
  
  // 캠프 모드에서 템플릿 블록 세트 ID 조회 (공통 함수 사용)
  const { getTemplateBlockSetId } = await import("@/lib/plan/blocks");
  const schedulerOptions: SchedulerOptions = (result.group.scheduler_options as SchedulerOptions | null) ?? {};
  const tenantBlockSetId = await getTemplateBlockSetId(
    result.group.camp_template_id,
    schedulerOptions
  );

  // 조회된 block_set_id 설정
  if (tenantBlockSetId) {
    creationData.block_set_id = tenantBlockSetId; // ❌ 외래 키 제약 조건 위반
  } else {
    console.warn(
      "[continueCampStepsForAdmin] 템플릿 블록 세트 ID를 찾을 수 없습니다:",
      result.group.camp_template_id
    );
  }
}
```

**변경 후**:
```typescript
creationData.plan_type = "camp";

// 캠프 모드에서는 block_set_id를 null로 설정
// 템플릿의 block_set_id는 tenant_block_sets 테이블의 ID이므로
// plan_groups.block_set_id (student_block_sets 참조)에 저장할 수 없음
creationData.block_set_id = null; // ✅ null로 설정

if (result.group.camp_template_id) {
  creationData.camp_template_id = result.group.camp_template_id;
  
  // 캠프 모드에서 템플릿 블록 세트 ID 조회 (공통 함수 사용)
  const { getTemplateBlockSetId } = await import("@/lib/plan/blocks");
  const schedulerOptions: SchedulerOptions = (result.group.scheduler_options as SchedulerOptions | null) ?? {};
  const tenantBlockSetId = await getTemplateBlockSetId(
    result.group.camp_template_id,
    schedulerOptions,
    tenantId // ✅ tenantId 전달
  );

  // 템플릿 블록 세트 ID를 scheduler_options에 저장
  if (tenantBlockSetId) {
    if (!creationData.scheduler_options) {
      creationData.scheduler_options = {};
    }
    type SchedulerOptionsWithTemplateBlockSet = SchedulerOptions & {
      template_block_set_id?: string;
    };
    (creationData.scheduler_options as SchedulerOptionsWithTemplateBlockSet).template_block_set_id =
      tenantBlockSetId; // ✅ scheduler_options에 저장
    console.log(
      "[continueCampStepsForAdmin] scheduler_options에 template_block_set_id 추가:",
      {
        template_block_set_id: tenantBlockSetId,
        scheduler_options: creationData.scheduler_options,
      }
    );
  } else {
    console.warn(
      "[continueCampStepsForAdmin] 템플릿 블록 세트 ID를 찾을 수 없습니다:",
      result.group.camp_template_id
    );
  }
}
```

## ✅ 수정 결과

### 수정 전

- ❌ 외래 키 제약 조건 위반 에러 발생
- ❌ 플랜 그룹 업데이트 실패
- ❌ 관리자가 남은 단계를 진행할 수 없음

### 수정 후

- ✅ 외래 키 제약 조건 위반 해결
- ✅ 플랜 그룹 업데이트 성공
- ✅ 템플릿 블록 세트 ID가 `scheduler_options.template_block_set_id`에 저장됨
- ✅ 관리자가 남은 단계를 정상적으로 진행할 수 있음

## 🔍 데이터 흐름

### 수정 전

```
continueCampStepsForAdmin
  ↓
tenantBlockSetId 조회 ✅
  ↓
creationData.block_set_id = tenantBlockSetId ❌
  ↓
plan_groups 업데이트 시도
  ↓
외래 키 제약 조건 위반 ❌
```

### 수정 후

```
continueCampStepsForAdmin
  ↓
creationData.block_set_id = null ✅
  ↓
tenantBlockSetId 조회 ✅
  ↓
scheduler_options.template_block_set_id = tenantBlockSetId ✅
  ↓
plan_groups 업데이트 성공 ✅
```

## 📝 주요 변경 사항

1. **`block_set_id`를 `null`로 설정**
   - 캠프 모드에서는 항상 `null`로 설정
   - 외래 키 제약 조건 위반 방지

2. **템플릿 블록 세트 ID를 `scheduler_options`에 저장**
   - `scheduler_options.template_block_set_id`에 저장
   - `campActions.ts`의 `submitCampParticipation`과 동일한 패턴

3. **`tenantId` 파라미터 추가**
   - `getTemplateBlockSetId` 호출 시 `tenantId` 전달
   - 조회 정확도 향상

## 🔗 관련 문서

- [캠프 템플릿 제출 시 블록 세트값 전달 검토](./20251222_캠프-템플릿-블록-세트-전달-검토.md)
- [관리자 페이지 '남은 단계 진행하기' 블록세트 로직 업데이트](./admin-camp-continue-block-set-logic-update.md)
- [플랜 그룹 시간 블록 기능 개선 Phase 1 완료 보고](./plan-group-time-block-improvement-phase1-completion-2025-02-01.md)


