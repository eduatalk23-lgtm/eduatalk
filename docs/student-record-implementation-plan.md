...
└────┬────┘    └────┬────┘    └────┬────┘    └────┬────┘
     │              │              │              │
 ┌───┴───┐    ┌────┴────┐   ┌────┴────┐   ┌────┴────┐
 │AI: PDF │    │AI: 역량  │   │AI: 보완  │   │ML: 졸업 │
 │Import  │    │태그 제안 │   │전략 제안 │   │생 매칭  │
...
- **전략**: 모평 배치 분석 (규칙 기반) + 졸업생 유사도 매칭 (Python ML)
...
| H | 졸업생 유사도 매칭 | ML | Python ML API | /api/analysis/compare | 8.6 | 검색 |
...
### 5.2 평가 세부질문 (루브릭) — Excel 이름정의 시트 기준
...
### 5.4 점수↔등급 환산표 — Excel 이름정의 시트 기준
...
### 5.5 전공 계열별 추천 교과목 — Excel 이름정의 시트 기준
...
### 6.1 `student_record_seteks` (교과 세특)
...
### 6.2 `student_record_personal_seteks` (개인 세특 — 학교자율과정)
...
### 6.3 `student_record_changche` (창체)
...
### 6.4 `student_record_haengteuk` (행특)
...
### 6.5 `student_record_reading` (독서활동)
...
### 6.6 `student_record_subject_pairs` (공통과목 쌍 참조)
...
### 6.7 `student_record_competency_scores` (역량 평가)
...
### 6.8 `student_record_activity_tags` (활동별 역량 태그)
...
### 6.9 `student_record_diagnosis` (종합 진단)
...
### 6.10 `student_record_strategies` (보완전략)
...
### 6.11 `student_record_attendance` (학교 출결 — NEIS 기준)
...
### 6.12 `student_record_applications` (지원 결과)
...
### 6.13 `student_record_awards` (수상경력)
...
### 6.14 `student_record_volunteer` (봉사활동)
...
### 6.15 `student_record_disciplinary` (징계사항)
...
### 6.12 RLS — SECURITY DEFINER 헬퍼 패턴
...
### 6.13 updated_at 트리거 — 테이블별 독립 함수
...
### 8.3 AI 파싱 스키마 (Gemini Structured Output)
...
├── alumni-matcher.ts             # [ML] 졸업생 유사도 매칭 (Python ML API)
├── api/
...
| **Python ML** | `lib/api/python-ml.ts` | /api/analysis/compare | alumni-matcher.ts |
