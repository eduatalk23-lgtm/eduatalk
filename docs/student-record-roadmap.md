...
| **M3: AI 진단** | 5~6.5 | 역량 태그 + 종합 진단 + 경보 | AI 기반 역량 분석 자동화 |
...
### Phase 6 — AI 종합 진단 + 스토리라인 분석

> **목표**: 역량 데이터 → AI 종합 진단 자동 생성 + 스토리라인 일관성 분석
...
```
6.1  llm/prompts/diagnosisGeneration.ts
     입력: 10개 역량 등급 + 활동 태그 + 성적 추이 + 진로 목표
     출력: overallGrade, recordDirection, strengths[], weaknesses[], recommendedMajors[]
6.2  llm/actions/generateDiagnosis.ts — LLM 캐시 168h
6.3  UI: DiagnosisGenerateButton → DiagnosisSummaryEditor 프리필
+    AI 스토리라인 분석: suggestStorylines + storylineAnalysis 프롬프트
```
