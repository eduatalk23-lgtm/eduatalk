...
| **M5: AI 고도화** | 9 | 활동 요약서 + Report 생성 | 수시 Report 자동 발행 (**9.1~9.3 완료**, 9.4 착수 가능 — C1 완료) |
| **M-CMS: 탐구 가이드** | C1~C5 | 7,836건 가이드 DB + AI 생성 | Access DB 완전 대체 (**C1 완료**) |
...
| 파일 | 내용 | 비고 |
|------|------|------|
| `학과조회4.accdb` | **최신 우회학과 Access DB** (188MB, 2026-01-14) | 서울 상위 10개 대학 |
| `# 이전/학과조회3_SKY.accdb` | SKY 버전 (91MB) | 이전 버전 |
| `READ ME.txt` | 대상 대학 목록 (서울대/연세대/고려대/건국대/성균관대/한양대/서강대/중앙대/경희대/한국외대) | |
| `대학 이수표준형태 및 교육과정편람/` | 대학별 교육과정 PDF 7건 | 우회학과 판단 근거 |

**대학별 교육과정 자료**:
- 서울대: 학과별 이수표준형태 (2024)
- 연세대: 대학요람 교과과정 (2025)
- 고려대: 교육과정편람 (2025, 114MB)
...
CMS C1+C1.1 ✅ (탐구 가이드 DB 7+2 테이블 + 7,836건 Import + 배정/검색/추천 Admin UI)
```
...
| **CMS C2** (가이드 CRUD UI + 에디터) | **다음 착수 권장** | C1.1 실사용 데이터 | 리치 텍스트 에디터 + 이미지 업로드 |
| **Agent Phase A** (AI SDK 마이그레이션) | 착수 가능 | 없음 | 기존 Gemini 추상화 안정적, 급하지 않음 |
| **Agent Phase C** (CMS RAG + pgvector) | 🟢 **착수 가능** | — (C1 완료) | C1 DB 기반 임베딩 파이프라인 구축 |
| Report P4 (rubric DB 정규화) | ~~미착수~~ 불필요 | — | 문자열 역매핑으로 충분 (9.1d에서 결정) |

### 추천 다음 작업

**CMS C1 + C1.1 + Phase 9.4 완료** (2026-03-20)로 다음 트랙이 열림:

**경로 1: CMS C2 (가이드 CRUD UI + 에디터)**
- 컨설턴트가 가이드를 직접 편집/생성하는 CMS 관리 UI
- TipTap 리치 텍스트 에디터 + Supabase Storage 이미지 업로드
- C1.1 실사용 데이터 기반 우선순위 결정

**경로 2: Agent Phase A → C (AI SDK + CMS RAG)**
- AI SDK v6 마이그레이션 후 pgvector 임베딩 파이프라인 구축
- 7,836건 가이드 벡터 검색 기반 추천 고도화

**경로 3: CMS C1.5 (우회학과 DB)**
- 학과조회4.accdb (188MB, 서울 상위 10개 대학) 이관 — 별도 독립 Phase
