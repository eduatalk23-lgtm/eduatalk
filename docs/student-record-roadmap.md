...
**도메인**: `lib/domains/bypass-major/` (types, repository, similarity-engine, actions 8개, import)
**Admin UI**: BypassMajorPanel + BypassTargetSelector + BypassCandidateList + CurriculumComparisonView
**CLI**: `scripts/import-departments.ts`

**남은**: 유사도 엔진 고도화 (Jaccard → 가중치 기반), 입시 배치 연동 (Phase 8.5)
...
**Agent + CMS 전 트랙 완료** (2026-03-20). 남은 작업:

**경로 1: Phase 8.4 (입시 DB 갱신 + 알림)**
- data.go.kr 기업 계정 확보 후 자동 갱신 파이프라인

**경로 2: 우회학과 고도화**
- 유사도 엔진: Jaccard → 가중치 기반 (전공필수 > 교양)
- 입시 배치 연동: Phase 8.5 PlacementResult + 우회학과 후보 결합
...
**전 트랙 완료** — CMS C1~C5, Agent A~E + 운영 안정화 모두 완료.
