# 캠프 템플릿 제출 시 블록 세트값 전달 검토

**작업 일시**: 2024년 12월 22일  
**작업자**: AI Assistant

## 📋 검토 개요

학생의 캠프 템플릿 제출 시, 템플릿에 설정되어 있던 블록 세트값이 올바르게 전달되는지 전체 데이터 흐름을 검토했습니다.

## 🔍 데이터 흐름 분석

### 1. 템플릿 블록 세트 ID 조회

**파일**: `app/(student)/actions/campActions.ts` (line 295-308)

```typescript
// 연결 테이블에서 템플릿에 연결된 블록 세트 조회
let templateBlockSetId: string | null = null;
const { data: templateBlockSetLink } = await supabase
  .from("camp_template_block_sets")
  .select("tenant_block_set_id")
  .eq("camp_template_id", template.id)
  .maybeSingle();

if (templateBlockSetLink) {
  templateBlockSetId = templateBlockSetLink.tenant_block_set_id;
} else if (templateData.block_set_id) {
  // 하위 호환성: template_data.block_set_id도 확인 (마이그레이션 전 데이터용)
  templateBlockSetId = templateData.block_set_id;
}
```

**검토 결과**: ✅ 정상
- 연결 테이블(`camp_template_block_sets`)에서 우선 조회
- 하위 호환성을 위해 `template_data.block_set_id`도 확인
- 두 경로 모두 정상적으로 처리됨

### 2. mergedData에 block_set_id 설정

**파일**: `app/(student)/actions/campActions.ts` (line 349)

```typescript
const mergedData: Partial<WizardData> = {
  ...templateData,
  block_set_id: wizardData.block_set_id || templateBlockSetId || "",
  // ... 기타 필드
};
```

**검토 결과**: ✅ 정상
- 학생이 선택한 블록 세트(`wizardData.block_set_id`)가 있으면 우선 사용
- 없으면 템플릿 블록 세트(`templateBlockSetId`) 사용
- 둘 다 없으면 빈 문자열 (빈 문자열은 나중에 null로 변환됨)

### 3. scheduler_options에 template_block_set_id 추가

**파일**: `app/(student)/actions/campActions.ts` (line 421-445)

```typescript
// 템플릿 블록 세트 ID를 scheduler_options에 먼저 추가
// syncWizardDataToCreationData 호출 전에 추가하여 보존되도록 함
const blockSetId = mergedData.block_set_id || templateBlockSetId;
console.log("[campActions] template_block_set_id 저장 준비:", {
  blockSetId,
  mergedData_block_set_id: mergedData.block_set_id,
  templateBlockSetId,
  mergedData_scheduler_options_before: mergedData.scheduler_options,
});

if (blockSetId) {
  if (!mergedData.scheduler_options) {
    mergedData.scheduler_options = {};
  }
  if (typeof mergedData.scheduler_options === "object" && mergedData.scheduler_options !== null) {
    (mergedData.scheduler_options as Record<string, unknown>).template_block_set_id = blockSetId;
  }
  console.log(
    "[campActions] mergedData.scheduler_options에 template_block_set_id 추가:",
    {
      template_block_set_id: blockSetId,
      scheduler_options: mergedData.scheduler_options,
    }
  );
}
```

**검토 결과**: ✅ 정상
- `syncWizardDataToCreationData` 호출 **전에** `scheduler_options.template_block_set_id`에 추가
- `mergedData.block_set_id` 또는 `templateBlockSetId` 중 하나를 사용
- 디버깅 로그 포함

### 4. syncWizardDataToCreationData 호출

**파일**: `app/(student)/actions/campActions.ts` (line 479-482)  
**변환 함수**: `lib/utils/planGroupDataSync.ts` (line 22-269)

```typescript
const { syncWizardDataToCreationData } = await import(
  "@/lib/utils/planGroupDataSync"
);
const creationData = syncWizardDataToCreationData(fullWizardData);
```

**변환 함수 내부 처리** (`lib/utils/planGroupDataSync.ts` line 44-63):

```typescript
// 1. scheduler_options 구성
const schedulerOptions: Record<string, unknown> = {
  ...(validatedData.scheduler_options || {}),
};

// study_review_cycle을 scheduler_options에 병합
if (validatedData.study_review_cycle) {
  schedulerOptions.study_days = validatedData.study_review_cycle.study_days;
  schedulerOptions.review_days = validatedData.study_review_cycle.review_days;
}

// time_settings를 scheduler_options에 안전하게 병합 (보호 필드 자동 보호)
let finalSchedulerOptions = schedulerOptions;
if (validatedData.time_settings) {
  finalSchedulerOptions = mergeTimeSettingsSafely(schedulerOptions, validatedData.time_settings);
}
```

**검토 결과**: ✅ 정상
- `validatedData.scheduler_options`를 spread하므로 `template_block_set_id`가 보존됨
- `mergeTimeSettingsSafely` 함수에서 `template_block_set_id`는 **보호 필드**로 처리됨
- 보호 필드 목록: `["template_block_set_id", "camp_template_id"]` (`lib/utils/schedulerOptionsMerge.ts` line 13)

### 5. 보호 필드 처리 확인

**파일**: `lib/utils/schedulerOptionsMerge.ts` (line 13, 87-100)

```typescript
const PROTECTED_FIELDS = ["template_block_set_id", "camp_template_id"];

// 보호 필드 추출 (schedulerOptions에 존재하는 경우만)
const protectedFields = Object.fromEntries(
  PROTECTED_FIELDS
    .filter((key) => schedulerOptions[key] !== undefined)
    .map((key) => [key, schedulerOptions[key]])
);

// 병합 (보호 필드 제외)
const merged = {
  ...schedulerOptions,
  ...timeSettings,
  ...protectedFields, // 보호 필드 재적용
};
```

**검토 결과**: ✅ 정상
- `template_block_set_id`는 보호 필드로 등록되어 있음
- `time_settings` 병합 시에도 보존됨
- 보호 필드가 재적용되어 덮어쓰이지 않음

### 6. 추가 확인 로직

**파일**: `app/(student)/actions/campActions.ts` (line 649-688)

```typescript
// 캠프 모드에서는 block_set_id를 null로 설정
// 템플릿의 block_set_id는 template_block_sets 테이블의 ID이므로
// plan_groups.block_set_id (student_block_sets 참조)에 저장할 수 없음
creationData.block_set_id = null;

// template_block_set_id는 이미 mergedData.scheduler_options에 추가되어 있음
// syncWizardDataToCreationData에서 scheduler_options를 병합할 때 보존됨
// 추가 확인: creationData.scheduler_options에 template_block_set_id가 있는지 확인
type SchedulerOptionsWithTemplateBlockSet = SchedulerOptions & {
  template_block_set_id?: string;
};

const schedulerOptions = creationData.scheduler_options as SchedulerOptionsWithTemplateBlockSet | null | undefined;
if (!schedulerOptions?.template_block_set_id && blockSetId) {
  console.warn(
    "[campActions] creationData.scheduler_options에 template_block_set_id가 없어 추가:",
    {
      blockSetId,
      creationData_scheduler_options: creationData.scheduler_options,
    }
  );
  if (!creationData.scheduler_options) {
    creationData.scheduler_options = {};
  }
  (creationData.scheduler_options as SchedulerOptionsWithTemplateBlockSet).template_block_set_id =
    blockSetId;
  console.log(
    "[campActions] creationData.scheduler_options에 template_block_set_id 추가 완료:",
    {
      scheduler_options: creationData.scheduler_options,
    }
  );
} else {
  console.log(
    "[campActions] creationData.scheduler_options에 template_block_set_id 확인됨:",
    {
      template_block_set_id: schedulerOptions?.template_block_set_id,
    }
  );
}
```

**검토 결과**: ✅ 정상
- `syncWizardDataToCreationData` 호출 후 `template_block_set_id` 존재 여부 확인
- 없으면 다시 추가하는 안전장치 포함
- 디버깅 로그 포함

### 7. DB 저장

**파일**: `lib/data/planGroups.ts` (line 380-422)

```typescript
export async function createPlanGroup(
  group: {
    // ... 기타 필드
    scheduler_options?: SchedulerOptions | null;
    // ...
  }
): Promise<{ success: boolean; groupId?: string; error?: string }> {
  const payload: PlanGroupPayload = {
    // ... 기타 필드
  };

  // scheduler_options가 실제 값이 있으면 추가 (데이터베이스에 컬럼이 있을 경우)
  // null이나 undefined가 아닐 때만 추가
  if (group.scheduler_options !== undefined && group.scheduler_options !== null) {
    payload.scheduler_options = group.scheduler_options;
  }

  // ... DB 저장 로직
}
```

**검토 결과**: ✅ 정상
- `scheduler_options`를 그대로 저장
- `template_block_set_id`가 포함된 상태로 저장됨

## ✅ 검토 결과 요약

### 정상 동작 확인

1. **템플릿 블록 세트 ID 조회**: ✅ 정상
   - 연결 테이블에서 우선 조회
   - 하위 호환성 경로도 확인

2. **데이터 병합**: ✅ 정상
   - 학생 선택값 우선, 템플릿값 fallback
   - `mergedData.block_set_id`에 올바르게 설정

3. **scheduler_options에 추가**: ✅ 정상
   - `syncWizardDataToCreationData` 호출 전에 추가
   - 보호 필드로 등록되어 덮어쓰이지 않음

4. **데이터 변환**: ✅ 정상
   - `syncWizardDataToCreationData`에서 보존됨
   - `mergeTimeSettingsSafely`에서 보호 필드로 처리됨

5. **안전장치**: ✅ 정상
   - 변환 후 추가 확인 로직
   - 없으면 다시 추가하는 fallback

6. **DB 저장**: ✅ 정상
   - `scheduler_options`에 `template_block_set_id` 포함하여 저장

### 데이터 흐름 다이어그램

```
템플릿 블록 세트 조회
  ↓
camp_template_block_sets 테이블 조회
  ↓
templateBlockSetId 설정
  ↓
mergedData.block_set_id 설정 (wizardData 우선)
  ↓
scheduler_options.template_block_set_id 추가 (보호 필드)
  ↓
syncWizardDataToCreationData 호출
  ↓
mergeTimeSettingsSafely (보호 필드 보존)
  ↓
추가 확인 로직 (fallback)
  ↓
DB 저장 (plan_groups.scheduler_options)
```

## 🔍 잠재적 이슈 및 개선 사항

### 현재 구현의 강점

1. **다중 경로 조회**: 연결 테이블 + 하위 호환성 경로
2. **보호 필드 메커니즘**: `template_block_set_id`가 덮어쓰이지 않도록 보호
3. **안전장치**: 변환 후 추가 확인 로직
4. **디버깅 로그**: 각 단계에서 상세한 로그 출력

### 개선 제안 (선택사항)

1. **타입 안전성 강화**
   - `SchedulerOptions` 타입에 `template_block_set_id` 필드 추가 고려
   - 현재는 타입 단언으로 처리 중

2. **에러 처리 강화**
   - 블록 세트 조회 실패 시 명확한 에러 메시지
   - 사용자에게 친화적인 에러 메시지 제공

3. **테스트 코드 추가**
   - 단위 테스트: `syncWizardDataToCreationData`에서 보호 필드 보존 확인
   - 통합 테스트: 전체 데이터 흐름 검증

## 📝 결론

**캠프 템플릿 제출 시 블록 세트값이 올바르게 전달됩니다.**

전체 데이터 흐름을 검토한 결과:
- 템플릿 블록 세트 ID가 정상적으로 조회됨
- `scheduler_options.template_block_set_id`에 올바르게 저장됨
- 보호 필드 메커니즘으로 덮어쓰이지 않음
- DB에 정상적으로 저장됨

현재 구현은 안정적이며, 보호 필드 메커니즘과 안전장치가 잘 작동하고 있습니다.

## 🔗 관련 파일

- `app/(student)/actions/campActions.ts` - 캠프 참여 정보 제출 로직
- `lib/utils/planGroupDataSync.ts` - WizardData → CreationData 변환
- `lib/utils/schedulerOptionsMerge.ts` - 보호 필드 처리
- `lib/data/planGroups.ts` - 플랜 그룹 생성 및 저장
- `lib/camp/campAdapter.ts` - 캠프 블록 세트 ID 조회 (3단계 우선순위)

## 📚 참고 문서

- [캠프 템플릿 블록 저장 및 조회 경로 분석](./camp-template-blocks-storage-analysis.md)
- [캠프 템플릿 제출 후 시간블록 표시 개선](./camp-submission-template-blocks-fix.md)
- [캠프 템플릿 학습 제외일 검증 개선](./20251222_055520-캠프-템플릿-학습-제외일-검증-개선.md)

