...
function LoginCardInner() {
  ...
  return (
    <div
      id="login-card"
      className="glass-card dark:glass-card-dark rounded-2xl p-6 shadow-xl w-full max-w-[400px]"
    >
        ...
        {ROLES.map((r) => (
          <button
            ...
            className={`flex-1 rounded-md py-2 text-sm font-medium transition-all ${
              activeRole === r.key
                ? "bg-white shadow text-gray-900 dark:bg-slate-600 dark:text-white"
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <div>
          ...
          <input
            ...
            className="w-full rounded-xl border border-gray-200 bg-white/50 px-4 py-3 text-sm outline-none transition-all focus:border-primary-400 focus:ring-2 focus:ring-primary-100 dark:border-slate-600 dark:bg-slate-800/50 dark:text-white dark:focus:border-primary-500 dark:focus:ring-primary-900/30"
          />
        </div>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <Link
            ...
            className="text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200 hover:underline"
          >
            비밀번호 찾기
          </Link>
        </div>
        ...
        <button
          type="submit"
          disabled={isPending}
          className="w-full rounded-xl bg-primary-600 px-4 py-3 text-white font-medium shadow-lg shadow-primary-600/20 transition-all hover:bg-primary-700 hover:shadow-xl active:scale-[0.98] disabled:opacity-50"
        >
          {isPending ? (
      ...
      <div className="relative my-5">
        ...
        <div className="relative flex justify-center text-xs">
          <span className="bg-white/70 dark:bg-slate-800/70 px-3 text-gray-400 dark:text-slate-500">
            또는
          </span>
        </div>
      </div>
...
export function LandingLoginCard() {
  return (
    <Suspense
      fallback={
        <div className="glass-card dark:glass-card-dark rounded-2xl p-6 shadow-xl w-full max-w-[400px] h-[520px] animate-pulse" />
      }
    >
      <LoginCardInner />
    </Suspense>
