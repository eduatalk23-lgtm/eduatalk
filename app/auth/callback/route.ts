 ...
 * OAuth 콜백에서 /join/[token] → OAuth → callback 플로우에서 호출
 */
async function processJoinToken(
  token: string,
  userId: string,
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      console.error("[auth/callback] 통합 초대 처리 중 에러:", error);
    }
    return { success: false };
  }
}
...
export async function GET(request: Request) {
  ...
  const joinToken = searchParams.get("join_token"); // 통합 초대 토큰
  ...
  // 쿠키에서 동의 데이터 읽기 (join 페이지에서 설정)
  let joinConsents: ConsentData | undefined;
  try {
    const cookieStore = await cookies();
    const consentsCookie = cookieStore.get("join_consents");
    if (consentsCookie?.value) {
      joinConsents = JSON.parse(consentsCookie.value);
    }
  } catch {
    // 쿠키 파싱 실패 시 무시
  }
  ...
  const redirectToNext = () => {
    ...
    // 동의 쿠키 정리
    response.cookies.delete("join_consents");
    return response;
  };
      ...
      if (user) {
        ...
        if (isOAuth) {
          ...
          if (!student && !parent && !admin) {
            // 통합 초대 토큰 처리
            if (joinToken) {
              const result = await processJoinToken(joinToken, user.id, joinConsents);
              if (result.success && result.redirectTo) {
                const sep = result.redirectTo.includes("?") ? "&" : "?";
                next = `${result.redirectTo}${sep}join_accepted=true`;
                return redirectToNext();
              }
              next = `/join/${joinToken}`;
              return redirectToNext();
