...
export const assignTenantToMultipleUsers = withActionResponse(_assignTenantToMultipleUsers);

/**
 * 활성 테넌트 목록 조회 (테넌트 할당용)
 */
async function _getActiveTenants(): Promise<Array<{ id: string; name: string }>> {
  const { role } = await getCurrentUserRole();
