...
async function _getCurriculumSettings(): Promise<CurriculumSettingsData> {
  ...
  if (role !== "superadmin") {
    throw new AppError("Super Admin만 접근할 수 있습니다.", ErrorCode.FORBIDDEN, 403, true);
  }
...
async function _updateCurriculumSettings(formData: FormData): Promise<void> {
  ...
  if (isNaN(middle_2022) || isNaN(high_2022) || isNaN(middle_2015) || isNaN(high_2015)) {
    throw new AppError("모든 년도를 입력해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }

  if (middle_2022 < 2000 || high_2022 < 2000 || middle_2015 < 2000 || high_2015 < 2000) {
    throw new AppError("유효한 년도를 입력해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }
...
export const updateCurriculumSettings = withActionResponse(_updateCurriculumSettings);
