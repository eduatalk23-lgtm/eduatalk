    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-xl border border-gray-30 bg-white p-6">
          <div className="text-body-2 text-gray-70">전체 사용자</div>
          <div className="mt-2 text-h2 text-gray-100">{stats.total}</div>
        </div>
        <div className="rounded-xl border border-gray-30 bg-white p-6">
          <div className="text-body-2 text-gray-70">학생</div>
          <div className="mt-2 text-h2 text-gray-100">{stats.students}</div>
        </div>
        <div className="rounded-xl border border-gray-30 bg-white p-6">
          <div className="text-body-2 text-gray-70">학부모</div>
          <div className="mt-2 text-h2 text-gray-100">{stats.parents}</div>
        </div>
        <div className="rounded-xl border border-gray-30 bg-white p-6">
          <div className="text-body-2 text-gray-70">미할당</div>
          <div className="mt-2 text-h2 text-gray-100">{stats.unassigned}</div>
        </div>
      </div>

      {/* 필터 및 검색 */}
      <div className="flex flex-col gap-4 rounded-xl border border-gray-30 bg-white p-6 md:flex-row md:items-center md:justify-between">
        ...
        <input
          ...
          className="w-full rounded-lg border border-gray-30 px-4 py-2 text-body-2 text-gray-100 md:w-64"
        />
      </div>

      {/* 사용자 목록 */}
      <div className="rounded-xl border border-gray-30 bg-white">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-gray-30 bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-body-2-bold text-gray-90">
                  이름
                </th>
                <th className="px-6 py-3 text-left text-body-2-bold text-gray-90">
                  이메일
            ...
            <tbody className="divide-y divide-gray-30">
              {filteredUsers.length === 0 ? (
                <tr>
                  <td
                    colSpan={6}
                    className="px-6 py-8 text-center text-body-2 text-gray-60"
                  >
                    {searchQuery
                      ? "검색 결과가 없습니다."
                      : "사용자가 없습니다."}
                ...
                filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-body-2 text-gray-100">
                      {user.name || "-"}
                    </td>
                    <td className="px-6 py-4 text-body-2 text-gray-100">
                      {user.email || "-"}
