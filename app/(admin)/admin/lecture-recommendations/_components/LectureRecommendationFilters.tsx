...
interface LectureRecommendationFiltersProps {
  platforms: string[];
 ...
 * 강의 추천 필터 컴포넌트
 */
export function LectureRecommendationFilters({
  platforms,
  subjectCategories,
  initialValues = {},
}: LectureRecommendationFiltersProps) {
  ...
  const applyFilters = useCallback(
    (updates: Record<string, string | undefined>) => {
      ...
      router.push(`/admin/lecture-recommendations?${params.toString()}`);
    },
    [router, searchParams]
  );
  ...
  const clearFilters = useCallback(() => {
    setSearch("");
    router.push("/admin/lecture-recommendations");
  }, [router]);
