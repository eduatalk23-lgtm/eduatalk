...
export function BaseMetadataManager<T extends BaseMetadataItem>({
  title,
  fetchAction,
  createAction,
  updateAction,
  ...
  async function loadItems() {
    ...
    try {
      ...
      toast.showError(`${title}을(를) 불러오는데 실패했습니다.`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleCreate() {
    ...
    try {
      await createAction(formData.name, formData.display_order);
      setFormData({ name: "", display_order: 0 });
      setIsCreating(false);
      toast.showSuccess(`${title}이(가) 생성되었습니다.`);
      loadItems();
    } catch (error) {
      console.error(`${title} 생성 실패:`, error);
      toast.showError(
  ...
  async function handleUpdate(id: string) {
    ...
    try {
      ...
      setEditingId(null);
      setFormData({ name: "", display_order: 0 });
      toast.showSuccess(`${title}이(가) 수정되었습니다.`);
      loadItems();
    } catch (error) {
      console.error(`${title} 수정 실패:`, error);
      toast.showError(
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      toast.showSuccess(`${title}이(가) 삭제되었습니다.`);
      loadItems();
    } catch (error) {
      console.error(`${title} 삭제 실패:`, error);
      toast.showError(
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => {
            setIsCreating(true);
            setEditingId(null);
            setFormData({ name: "", display_order: getInitialDisplayOrder(items) });
          }}
