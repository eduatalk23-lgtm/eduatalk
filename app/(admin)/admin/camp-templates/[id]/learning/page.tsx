...
import { CampLearningDashboard } from "./_components/CampLearningDashboard";

export default async function CampLearningPage({
  params,
}: {
  ...
  try {
    // 학습 통계 데이터 프리패칭
    await queryClient.prefetchQuery(campLearningStatsQueryOptions(id));
  } catch (error) {
    // Prefetch 실패 시에도 페이지는 렌더링되도록 에러만 로깅
    console.error("[CampLearningPage] 학습 통계 프리패칭 실패", error);
  }

  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <CampLearningDashboard template={result.template} templateId={id} />
    </HydrationBoundary>
  );
}
