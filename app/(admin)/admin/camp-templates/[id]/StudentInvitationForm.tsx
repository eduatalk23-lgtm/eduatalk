...
export function StudentInvitationForm({ templateId, templateStatus, onInvitationSent, refreshKey }: StudentInvitationFormProps) {
  ...
  const performSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      console.error("[StudentInvitationForm] 검색 오류:", error);
      setSearchResults([]);
    } finally {
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1">
            <input
              type="text"
              placeholder="이름 또는 연락처 4자리 이상 검색..."
              value={filter.search || ""}
              onChange={(e) => setFilter((prev) => ({ ...prev, search: e.target.value }))}
              disabled={isDisabled}
              className="w-full rounded-md border border-gray-300 px-3 py-2 pr-8 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-500"
            />
            {isSearching && (
