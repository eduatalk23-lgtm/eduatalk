...
import { fetchPendingAiGuidesAction } from "@/lib/domains/guide/actions/crud";
...
export default async function PendingApprovalPage() {
  const res = await fetchPendingAiGuidesAction(100);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-body-2 text-[var(--text-secondary)]">
            AI 파이프라인이 설계·생성한 가이드를 검수하고 승인하거나 거부합니다.
          </p>
        </div>
      ...
      <PendingApprovalClient initialGuides={guides} />
    </div>
  );
}
