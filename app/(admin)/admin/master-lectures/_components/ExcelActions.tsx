...
import { exportMasterLecturesToExcel, downloadMasterLecturesTemplate } from "@/lib/domains/master-content/actions/lectures/export";
...
export default function ExcelActions() {
  ...
  async function handleImport(file: File): Promise<{ success: boolean; message: string; errors?: string[] }> {
    ...
    const result = await importMasterLecturesFromExcel(uint8Array);
    if (isSuccessResponse(result) && result.data) {
      return {
        success: true,
        message: result.message || `${result.data.count}개의 강의가 업로드되었습니다.`,
