...
function HaengteukGridCell({
  ...
  if (column === "direction") {
    ...
    return (
      <div className="flex flex-col gap-1.5">
        ...
        {guideItem.evaluationItems && guideItem.evaluationItems.length > 0 && (
          <div className="mt-2 flex flex-col gap-1">
            <span className="text-xs font-medium text-[var(--text-secondary)]">평가 항목</span>
            {guideItem.evaluationItems.map((ev, i) => (
  ...
  // ── 가이드 (3행 분리: AI=추천배정, 컨설턴트=배정목록, 확정=완료) ──
  if (column === "guide") {
    if (perspective === "ai") {
...
function HaengteukDraftGridCell({
  perspective,
  record,
  charLimit,
  studentId,
...
function HaengteukAnalysisGridCell({
  perspective,
  record,
  tags,
  studentId,
