...
function EdgeRow({ edge, onDrillDown }: { edge: CrossRefEdge; onDrillDown: (key: string) => void }) {
  ...
  return (
    ...
    >
      {isInferred && (
        <span className="shrink-0 rounded-sm bg-violet-500 px-1 text-[9px] font-bold text-white">
          추론
        </span>
      )}
      {isProjected && (
        <span className="shrink-0 rounded-sm bg-amber-500 px-1 text-[9px] font-bold text-white">
          예상
        </span>
      )}
