...
import { importTimeManagementAction } from "@/lib/domains/admin-plan/actions/calendarEvents";
import type { AcademySchedule } from "./admin-wizard/_context/types";
...
export function PlannerCreationModal({
  ...
  // 학원 일정 관련 상태 (edit/duplicate 모드에서 submit에 전달)
  const [academySchedules, setAcademySchedules] = useState<AcademySchedule[]>([]);

  // 캘린더 통합: 초기 데이터 옵션
  const [importTimeManagementData, setImportTimeManagementData] = useState(true);
  const [importNonStudyDefaults, setImportNonStudyDefaults] = useState(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result) {
        // 신규 생성 시 시간관리 데이터 자동 가져오기
        if (mode === "create" && importTimeManagementData && result.id) {
          try {
            await importTimeManagementAction(result.id, studentId);
          } catch (importErr) {
            console.warn("[PlannerCreationModal] 시간관리 데이터 가져오기 실패:", importErr);
          }
        }
            ...
            <section className="border-t pt-6">
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-100">
                  <input
                    type="checkbox"
                    checked={importTimeManagementData}
                    onChange={(e) => setImportTimeManagementData(e.target.checked)}
                    className="h-4 w-4 rounded border-gray-300 text-blue-600"
                  />
                  <div>
                    <span className="text-sm font-medium text-gray-900">시간관리 데이터 가져오기</span>
                    <p className="text-xs text-gray-500">학원 일정 + 제외일을 캘린더에 자동 등록</p>
                  </div>
                </label>
                <label className="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-100">
                  <input
                    type="checkbox"
                    checked={importNonStudyDefaults}
                    onChange={(e) => setImportNonStudyDefaults(e.target.checked)}
                    className="h-4 w-4 rounded border-gray-300 text-blue-600"
                  />
                  <div>
                    <span className="text-sm font-medium text-gray-900">기본 비학습시간 자동 생성</span>
                    <p className="text-xs text-gray-500">점심식사 등 비학습시간 레코드 자동 생성</p>
                  </div>
                </label>
                <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <Info className="w-4 h-4 text-blue-500 flex-shrink-0 mt-0.5" />
                  <p className="text-xs text-blue-700">
                    플래너 생성 후 설정 탭의 캘린더에서 학원 일정/제외일을 세부 조정할 수 있습니다.
                  </p>
                </div>
              </div>
            </section>
