...
import type { AcademySchedule } from "../_context/types";

type AdminAcademyScheduleImportModalProps = {
  isOpen: boolean;
  onClose: () => void;
...
/**
 * 관리자용 학원 일정 불러오기 모달
 * - 학생의 시간 관리 데이터에서 학원 일정 선택
 * - 학원별 그룹화 표시
 * - 체크박스로 다중 선택
 * - 겹치는 시간대 경고 표시
 */
export function AdminAcademyScheduleImportModal({
  isOpen,
  onClose,
  availableSchedules,
  existingSchedules,
  onImport,
}: AdminAcademyScheduleImportModalProps) {
  ...
  }, [groupedByAcademy, expandedAcademies.size]);
