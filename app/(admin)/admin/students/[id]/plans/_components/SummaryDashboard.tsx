...
export function SummaryDashboard({ studentId, className }: SummaryDashboardProps) {
  ...
  return (
    <div className={cn('bg-white dark:bg-[rgb(var(--color-secondary-50))] rounded-lg border overflow-hidden', className)}>
      {/* 헤더 */}
      <div className="flex items-center justify-between px-4 py-3 border-b">
        <h3 className="font-bold text-gray-800 dark:text-gray-200">학습 요약</h3>
        <div className="flex items-center gap-1 border rounded p-0.5">
          <button
            onClick={() => setTimeRange('week')}
            className={cn(
              'px-2 py-1 text-xs rounded transition-colors',
              timeRange === 'week' ? 'bg-blue-500 text-white' : 'text-gray-600 dark:text-gray-400'
            )}
          >
            주간
          </button>
          <button
            ...
            className={cn(
              'px-2 py-1 text-xs rounded transition-colors',
              timeRange === 'month' ? 'bg-blue-500 text-white' : 'text-gray-600 dark:text-gray-400'
            )}
          >
            월간
          </button>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{stats.totalPlans}</div>
            <div className="text-xs text-gray-600 dark:text-gray-400">전체 플랜</div>
          </div>
          ...
          <div className="bg-purple-50 rounded-lg p-3 text-center">
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400">완료 분량</div>
          </div>
          <div className="bg-amber-50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-amber-600">{stats.carryoverCount}</div>
            <div className="text-xs text-gray-600 dark:text-gray-400">이월 건수</div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            <span className="text-gray-600 dark:text-gray-400">플랜 진행률</span>
          ...
          <div className="flex justify-between text-sm mt-3">
            <span className="text-gray-600 dark:text-gray-400">분량 진행률</span>
        ...
        {/* 일별 트렌드 (미니 차트) */}
        <div>
          <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">일별 완료 추이</div>
            ...
            {stats.weeklyTrend.map((day, i) => {
              ...
              return (
                <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-t relative" style={{ height: '48px' }}>
                    <div
                      className={cn(
                        'absolute bottom-0 left-0 right-0 rounded-t transition-all',
                        height >= 100 ? 'bg-green-500' : height > 0 ? 'bg-blue-400' : 'bg-gray-200 dark:bg-gray-700'
                      )}
                      style={{ height: `${Math.max(height, 4)}%` }}
                    />
                  </div>
                  <span className="text-[10px] text-gray-500 dark:text-gray-400">{dayLabel}</span>
                </div>
              );
            })}
          </div>
        ...
        {stats.subjectBreakdown.length > 0 && (
          <div>
            <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">과목별 현황</div>
              ...
              {stats.subjectBreakdown.slice(0, 5).map((item) => {
                ...
                return (
                  <div key={item.subject} className="flex items-center gap-2">
                    <span className="text-xs text-gray-600 dark:text-gray-400 w-16 truncate">{item.subject}</span>
