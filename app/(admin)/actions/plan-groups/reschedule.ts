/**
 * 관리자용 플랜 그룹 재조정 Server Actions
 * 
 * 관리자가 학생의 플랜 그룹을 재조정할 수 있는 기능을 제공합니다.
 * 
 * @module app/(admin)/actions/plan-groups/reschedule
 */
 ...
 * @returns 미리보기 결과
 */
async function _getReschedulePreviewForAdmin(
  groupId: string,
...
export const getReschedulePreviewForAdmin = withErrorHandling(_getReschedulePreviewForAdmin);
 ...
 * @returns 실행 결과
 */
async function _rescheduleContentsForAdmin(
  groupId: string,
  adjustments: AdjustmentInput[],
  reason?: string,
  rescheduleDateRange?: { from: string; to: string } | null,
  ...
  const previewResult = await _getReschedulePreviewForAdmin(
    groupId,
    adjustments,
    rescheduleDateRange,
    placementDateRange,
...
export const rescheduleContentsForAdmin = withErrorHandling(_rescheduleContentsForAdmin);
