...
export function Step6FinalReview({
      ...
      for (const content of data.student_contents) {
        ...
        if (!title || !subjectCategory) {
          try {
            ...
            console.error(
              "[Step6FinalReview] 학생 콘텐츠 메타데이터 조회 실패:",
              planGroupError
            );
          }
        }
      ...
      for (const content of data.recommended_contents) {
        ...
        if (!title || !subjectCategory) {
          try {
            ...
            console.error(
              "[Step6FinalReview] 마스터 콘텐츠 메타데이터 조회 실패:",
              planGroupError
            );
          }
        }
  ...
  useEffect(() => {
    ...
    contentInfos.forEach((info) => {
      ...
      const content = contentKey.startsWith("student-")
    ...
    const fetchContentTotals = async () => {
      ...
      for (const contentInfo of contentInfos) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (!total) {
              ...
              try {
                ...
                console.error(
                  `[Step6FinalReview] 콘텐츠 ${contentInfo.content_id} 상세정보 조회 실패 (총량 추정용):`,
                  planGroupError
                );
              }
            }
          ...
          console.error(
            `[Step6FinalReview] 콘텐츠 ${contentInfo.content_id} 총량 조회 실패:`,
            planGroupError
          );
        }
      }
      ...
      if (startDetail && endDetail) {
        ...
        if (endDetailIndex >= 0 && endDetailIndex < details.length - 1) {
          ...
          if (total && newEnd < total) {
            ...
            // 단, 상세 정보의 최대 페이지와 비교
            const maxPageInDetails = Math.max(
              ...details.map((d) => d.page_number)
            );
                              ...
                              onClick={() => {
                                if (info.isRecommended) {
                                  ...
                                  if (
                                    !isNaN(index) &&
                                    index >= 0 &&
                                    index < data.recommended_contents.length
                                  ) {
                                    const updated = [
                                      ...data.recommended_contents,
                                    ];
                                  ...
                                  if (
                                    !isNaN(index) &&
                                    index >= 0 &&
                                    index < data.student_contents.length
                                  ) {
                ...
                >
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-600">
                      ...
                      {(() => {
                        ...
                        if (recommendedRange) {
                          const studentRange =
                            content.end_range - content.start_range + 1;
                          const recommendedRangeValue =
                            recommendedRange.end - recommendedRange.start + 1;
                          const difference =
                            studentRange - recommendedRangeValue;
                          return (
                            <>
                              <span>·</span>
                              <span
                              ...
                              {isLoading ? (
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <input
                                    ...
                                    min={1}
                                    value={
                                      editingRange?.end || content.end_range
                                    }
                                    onChange={(e) =>
                                      setEditingRange({
                                        start:
                  ...
                  {!isEditing &&
                    (() => {
                      ...
                      const studentRange =
                        content.end_range - content.start_range + 1;
                      const recommendedRangeValue = recommendedRange
                        ? recommendedRange.end - recommendedRange.start + 1
                        : null;
                      const difference =
                        recommendedRangeValue !== null
                          ? studentRange - recommendedRangeValue
                          : null;
                        ...
                        <div className="ml-4 flex flex-col items-end gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const updated = data.student_contents.filter(
                                  (_, i) => i !== index
                                );
                                onUpdate({ student_contents: updated });
                              }}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {data.recommended_contents.map((content, index) => {
              ...
              const isEditing =
                editingRangeIndex?.type === "recommended" &&
                editingRangeIndex.index === index;
                        ...
                        <div className="ml-4 flex flex-col items-end gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              onClick={() => {
                                const updated =
                                  data.recommended_contents.filter(
                                    (_, i) => i !== index
                                  );
                                onUpdate({ recommended_contents: updated });
                              }}
