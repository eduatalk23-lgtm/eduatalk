...
export function Step7ScheduleResult({
  ...
  // L2: 재생성 필요 알림 닫기 상태
  const [dismissedDirtyWarning, setDismissedDirtyWarning] = useState(false);
  ...
  // L2: isDirty 상태일 때 재생성 필요 알림
  // 플랜이 이미 있지만 데이터가 변경된 경우, 사용자에게 알림
  const showDirtyWarning = isDirty && plansCheck?.hasPlans && !dismissedDirtyWarning && !isGenerating;

  // 성공 축하 UI 표시 (첫 진입 시)
  // isDirty 상태에서는 축하 UI 대신 경고 배너 표시
  if (showCelebration && plansCheck.hasPlans && !showDirtyWarning) {
    return (
      <div className="flex flex-col gap-6">
        <SuccessCelebration
          groupId={groupId}
      ...
      {showDirtyWarning && (
        <div className="flex flex-col gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-semibold text-amber-800">설정이 변경되었습니다</h3>
              <p className="text-sm text-amber-700">
                이전 단계에서 설정을 변경했습니다. 현재 플랜은 이전 설정으로 생성되어 있습니다.
                변경된 설정을 반영하려면 플랜을 재생성하세요.
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            <button
              ...
              onClick={handleRegenerate}
              disabled={generatePlansMutation.isPending}
              className="inline-flex items-center gap-1.5 rounded-md bg-amber-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-amber-700 disabled:cursor-not-allowed disabled:bg-amber-400"
            >
              <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
            ...
            <button
              type="button"
              onClick={() => setDismissedDirtyWarning(true)}
