...
async function _createStudentPlan(formData: FormData): Promise<void> {
...
export const createStudentPlan = withErrorHandling(_createStudentPlan);

async function _updateStudentPlan(formData: FormData): Promise<void> {
  const user = await getCurrentUser();
  if (!user || user.role !== "student") {
    throw new AppError("로그인이 필요합니다.", ErrorCode.UNAUTHORIZED, 401, true);
  }
...
export const updateStudentPlan = withErrorHandling(_updateStudentPlan);

async function _deleteStudentPlan(id: string): Promise<void> {
  const user = await getCurrentUser();
  if (!user || user.role !== "student") {
    throw new AppError("로그인이 필요합니다.", ErrorCode.UNAUTHORIZED, 401, true);
  }
...
export const deleteStudentPlan = withErrorHandling(_deleteStudentPlan);
