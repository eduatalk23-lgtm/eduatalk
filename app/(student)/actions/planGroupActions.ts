...
import { assignPlanTimes, type PlanTimeSegment } from "@/lib/plan/assignPlanTimes";

/**
 * plan_purpose 값을 데이터베이스 스키마에 맞게 변환
 * "모의고사(수능)" -> "수능"
 */
function normalizePlanPurpose(purpose: string | null | undefined): string | null {
 ...
 * 플랜 그룹 생성 (JSON 데이터)
 */
async function _createPlanGroup(data: PlanGroupCreationData): Promise<{ groupId: string }> {
 ...
 */
async function _savePlanGroupDraft(data: PlanGroupCreationData): Promise<{ groupId: string }> {
  const user = await getCurrentUser();
  if (!user || user.role !== "student") {
    throw new AppError("로그인이 필요합니다.", ErrorCode.UNAUTHORIZED, 401, true);
  }
  ...
  if (!data.name || data.name.trim() === "") {
    throw new AppError("플랜 이름을 입력해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }
...
export const updatePlanGroupDraftAction = withErrorHandling(_updatePlanGroupDraft);
...
async function _updatePlanGroupStatus(
  ...
  if (status === "active") {
    ...
    if (activeGroupsError) {
      console.error("[planGroupActions] 활성 플랜 그룹 조회 실패", activeGroupsError);
      ...
      if (deactivateError) {
        console.error("[planGroupActions] 다른 활성 플랜 그룹 비활성화 실패", deactivateError);
        // 비활성화 실패해도 계속 진행 (경고만)
      } else {
        console.log(`[planGroupActions] ${activeGroupIds.length}개의 활성 플랜 그룹을 비활성화했습니다.`);
      }
    }
  }
...
async function _syncTimeManagementExclusions(
  ...
  // 플랜 그룹 조회
  const group = await getPlanGroupById(groupId, user.userId, tenantContext.tenantId);
  if (!group) {
    throw new AppError(
      "플랜 그룹을 찾을 수 없습니다.",
      ErrorCode.NOT_FOUND,
  ...
  const allExclusions = await getStudentExclusions(user.userId, tenantContext.tenantId);
...
export const syncTimeManagementExclusionsAction = withErrorHandling(_syncTimeManagementExclusions);
...
async function _syncTimeManagementAcademySchedules(groupId: string): Promise<{ count: number }> {
  ...
  const allAcademySchedules = await getStudentAcademySchedules(user.userId, tenantContext.tenantId);
...
export const syncTimeManagementAcademySchedulesAction = withErrorHandling(_syncTimeManagementAcademySchedules);
...
async function _deletePlanGroup(groupId: string): Promise<void> {
  ...
  try {
    ...
    const { data: progressData } = planIds.length > 0
      ? await supabase
    ...
    */
    ...
    console.log("[planGroupActions] 플랜 그룹 삭제 백업 데이터:", JSON.stringify(backupData, null, 2));
  } catch (backupError) {
  ...
  if (deletePlansError) {
    ...
    console.log(`[planGroupActions] 플랜 그룹 ${groupId}의 관련 플랜이 삭제되었습니다.`);
  }
 ...
 * 플랜 그룹에서 개별 플랜 생성
 */
async function _generatePlansFromGroup(groupId: string): Promise<{ count: number }> {
  const user = await getCurrentUser();
  if (!user || user.role !== "student") {
    throw new AppError("로그인이 필요합니다.", ErrorCode.UNAUTHORIZED, 401, true);
  }
  ...
  const { calculateAvailableDates } = await import("@/lib/scheduler/calculateAvailableDates");
  ...
  const dateMetadataMap = new Map<string, {
    ...
    if (content.content_type === "book") {
      ...
      if (masterBook) {
        ...
        try {
          ...
          console.error(`[planGroupActions] 마스터 교재 복사 실패: ${content.content_id}`, error);
          // 복사 실패 시 원본 ID 사용
        }
      }
      ...
      if (masterLecture) {
        ...
        try {
          ...
          console.error(`[planGroupActions] 마스터 강의 복사 실패: ${content.content_id}`, error);
          // 복사 실패 시 원본 ID 사용
        }
      }
    }
  ...
  const contentMetadataMap = new Map<string, { 
    title?: string | null;
    subject?: string | null; 
  ...
  // 8. 콘텐츠 소요시간 정보 조회
  const contentDurationMap = new Map<string, {
  ...
  if (existingPlans && existingPlans.length > 0) {
    console.log(`[planGroupActions] ${existingPlans.length}개의 기존 플랜 삭제 완료`);
    ...
    if (verifyError) {
      ...
      console.warn(`[planGroupActions] 경고: ${verifyPlans.length}개의 플랜이 아직 남아있습니다. 재삭제 시도...`);
      // 재삭제 시도
      await supabase
        .from("student_plan")
        .delete()
    ...
    try {
      if (contentType === "book") {
        ...
        if (studentBookDetail?.major_unit || studentBookDetail?.minor_unit) {
          ...
          return studentBookDetail.major_unit || studentBookDetail.minor_unit || null;
        }
        ...
        if (book?.master_content_id) {
          ...
          if (masterBookDetail?.major_unit || masterBookDetail?.minor_unit) {
            ...
            return masterBookDetail.major_unit || masterBookDetail.minor_unit || null;
          }
        }
      ...
      console.error(`[planGroupActions] 콘텐츠 메타데이터 조회 실패: ${contentId}`, error);
    }
    return null;
  };
  ...
  const { data: existingPlansForDates, error: existingPlansError } = await supabase
    .from("student_plan")
  ...
  if (existingPlansForDates) {
    existingPlansForDates.forEach((plan) => {
      if (plan.plan_date && plan.block_index !== null && plan.block_index !== undefined) {
  ...
  for (const [date, datePlans] of plansByDate.entries()) {
    ...
    // 날짜별 메타데이터 가져오기
    const dateMetadata = dateMetadataMap.get(date) || { day_type: null, week_number: null };
    ...
    const plansForAssign = datePlans.map((plan) => {
      const finalContentId = contentIdMap.get(plan.content_id) || plan.content_id;
      return {
    ...
    for (const segment of timeSegments) {
      ...
      const originalContentId = datePlans.find(
        (p) => p.content_id === segment.plan.content_id || 
    ...
    for (const slot of nonStudySlots) {
      ...
      if (nonStudySlots.indexOf(slot) === 0) {
        ...
        if (!existingDummyContent) {
          ...
          if (createError) {
            ...
            console.warn("[planGroupActions] 더미 custom content 생성 실패 (무시됨):", createError.message);
          }
        }
      }
      ...
      if (slot.type === "학원일정") {
        ...
        const dailySchedule = scheduleResult.daily_schedule.find((d) => d.date === date);
        if (dailySchedule?.academy_schedules && dailySchedule.academy_schedules.length > 0) {
          // 같은 시간대의 학원일정 찾기
          const matchingAcademy = dailySchedule.academy_schedules.find(
            (academy) => academy.start_time === slot.start && academy.end_time === slot.end
          );
          if (matchingAcademy) {
    ...
    if (dateMetadata.day_type === "지정휴일" && studyTimeSlots.length > 0 && enableSelfStudyForHolidays) {
      ...
      if (!existingSelfStudyContent) {
        ...
        if (createError) {
          // 생성 실패 시 경고만 출력 (더미 content는 선택사항)
          // contentDurationMap에 이미 기본값이 있으므로 플랜 생성은 계속 진행
          console.warn("[planGroupActions] 더미 자율학습 custom content 생성 실패 (무시됨):", createError.message);
        }
      }
  ...
  const plansNeedingChapter = planPayloads.filter((p) => 
    !p.chapter && 
  ...
  planPayloads.forEach((plan) => {
    ...
    if (planKeys.has(key)) {
      duplicatePlans.push({ plan_date: plan.plan_date, block_index: plan.block_index });
    } else {
      planKeys.add(key);
    }
  });
  ...
  if (duplicatePlans.length > 0) {
    console.error("[planGroupActions] 생성하려는 플랜 중 중복 발견:", duplicatePlans);
    throw new AppError(
  ...
  const regularPlans = planPayloads.filter(
    (p) => p.content_id !== DUMMY_NON_LEARNING_CONTENT_ID && p.content_id !== DUMMY_SELF_STUDY_CONTENT_ID
  );
    ...
    if (insertError) {
      ...
      if (insertError.code === "23505") {
        ...
        if (duplicatePlanData) {
          console.error("[planGroupActions] 현재 존재하는 플랜 (일부):", duplicatePlanData);
        }
  ...
  if (dummyPlans.length > 0) {
    ...
    if (dummyInsertError) {
      ...
      if (
        ...
        console.warn("[planGroupActions] 더미 UUID 플랜 저장 실패 (무시됨):", dummyInsertError.message);
  ...
  const dailyScheduleForStorage = scheduleResult.daily_schedule.map((daily) => ({
    date: daily.date,
  ...
  if (updateScheduleError) {
    console.error("[planGroupActions] dailySchedule 저장 실패", updateScheduleError);
...
export const generatePlansFromGroupAction = withErrorHandling(_generatePlansFromGroup);
...
async function _previewPlansFromGroup(groupId: string): Promise<{
  ...
  for (const content of contents) {
    ...
    try {
      ...
      console.error(`[planGroupActions] 마스터 콘텐츠 확인 실패: ${content.content_id}`, error);
      // 에러 발생 시 원본 ID 사용
      contentIdMap.set(content.content_id, content.content_id);
    }
  }
  ...
  // 더미 UUID에 대한 기본값 추가 (비학습 항목 및 자율학습용)
  const DUMMY_NON_LEARNING_CONTENT_ID_PREVIEW = "00000000-0000-0000-0000-000000000000";
  const DUMMY_SELF_STUDY_CONTENT_ID_PREVIEW = "00000000-0000-0000-0000-000000000001";
...
export const previewPlansFromGroupAction = withErrorHandling(_previewPlansFromGroup);
...
async function _getScheduleResultData(groupId: string): Promise<{
  ...
  if (!group) {
    console.error("[planGroupActions] 플랜 그룹을 찾을 수 없음:", { groupId, userId: user.userId });
    throw new AppError(
  ...
  // 강의 조회
  const lecturePlans = (plans || []).filter((p) => p.content_type === "lecture");
  ...
  if (group.block_set_id) {
    ...
    if (blockData && blockData.length > 0) {
      ...
      blocks = Array.from(blocksByDay.entries())
...
export const getScheduleResultDataAction = withErrorHandling(_getScheduleResultData);
...
async function _addPlanExclusion(formData: FormData): Promise<void> {
  ...
  if (!exclusionDate || typeof exclusionDate !== "string") {
    throw new AppError("제외일을 입력해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }

  if (!exclusionType || typeof exclusionType !== "string") {
    throw new AppError("제외 유형을 선택해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }

  // plan_group_id가 제공되지 않으면 활성 플랜 그룹 찾기
  let targetGroupId = planGroupId && typeof planGroupId === "string" ? planGroupId : null;
...
async function _deletePlanExclusion(formData: FormData): Promise<void> {
  ...
  if (!exclusionId || typeof exclusionId !== "string") {
    throw new AppError("제외일 ID가 필요합니다.", ErrorCode.VALIDATION_ERROR, 400, true);
  }
  ...
  if (fetchError || !exclusion) {
    throw new AppError("제외일을 찾을 수 없습니다.", ErrorCode.NOT_FOUND, 404, true);
  }
  ...
  const { error } = await supabase.from("plan_exclusions").delete().eq("id", exclusionId);
...
async function _addAcademySchedule(formData: FormData): Promise<void> {
  ...
  if (!dayOfWeek || typeof dayOfWeek !== "string") {
    throw new AppError("요일을 선택해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }

  if (!startTime || typeof startTime !== "string") {
    throw new AppError("시작 시간을 입력해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }

  if (!endTime || typeof endTime !== "string") {
    throw new AppError("종료 시간을 입력해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }

  if (!academyName || typeof academyName !== "string" || !academyName.trim()) {
    throw new AppError("학원 이름을 입력해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }

  if (!subject || typeof subject !== "string" || !subject.trim()) {
    throw new AppError("과목을 입력해주세요.", ErrorCode.VALIDATION_ERROR, 400, true);
  }
  ...
  if (startTime >= endTime) {
    throw new AppError("종료 시간은 시작 시간보다 늦어야 합니다.", ErrorCode.VALIDATION_ERROR, 400, true);
  }
    ...
    [
      {
        ...
        end_time: endTime,
        academy_name: academyName && typeof academyName === "string" ? academyName.trim() : null,
...
async function _updateAcademySchedule(formData: FormData): Promise<void> {
  ...
  if (!scheduleId || typeof scheduleId !== "string") {
    throw new AppError("학원 일정 ID가 필요합니다.", ErrorCode.VALIDATION_ERROR, 400, true);
  }
  ...
  if (fetchError || !schedule) {
    throw new AppError("학원 일정을 찾을 수 없습니다.", ErrorCode.NOT_FOUND, 404, true);
  }
...
async function _deleteAcademySchedule(formData: FormData): Promise<void> {
  ...
  const { error } = await supabase.from("academy_schedules").delete().eq("id", scheduleId);
 ...
 * 플랜 그룹 학원 일정 조회
 */
async function _getAcademySchedules(planGroupId: string): Promise<AcademySchedule[]> {
  const user = await getCurrentUser();
  if (!user || user.role !== "student") {
    throw new AppError("로그인이 필요합니다.", ErrorCode.UNAUTHORIZED, 401, true);
  }
  ...
  const group = await getPlanGroupById(planGroupId, user.userId, tenantContext?.tenantId || null);
  if (!group) {
    throw new AppError("플랜 그룹을 찾을 수 없습니다.", ErrorCode.NOT_FOUND, 404, true);
  }

  // 학생별 전역 학원 일정 조회
  const schedules = await getStudentAcademySchedules(user.userId, tenantContext?.tenantId || null);
  return schedules;
}
...
async function _createAcademy(formData: FormData): Promise<void> {
  ...
  const travelTimeNum = travelTime ? Number.parseInt(String(travelTime), 10) : 60;
  if (Number.isNaN(travelTimeNum) || travelTimeNum < 0) {
    throw new AppError("이동시간은 0 이상의 숫자여야 합니다.", ErrorCode.VALIDATION_ERROR, 400, true);
  }
  ...
  if (existingAcademy) {
    throw new AppError("이미 같은 이름의 학원이 등록되어 있습니다.", ErrorCode.VALIDATION_ERROR, 400, true);
  }
...
async function _updateAcademy(formData: FormData): Promise<void> {
  ...
  if (!academyId || typeof academyId !== "string") {
    throw new AppError("학원 ID가 필요합니다.", ErrorCode.VALIDATION_ERROR, 400, true);
  }
  ...
  if (fetchError || !academy) {
    throw new AppError("학원을 찾을 수 없습니다.", ErrorCode.NOT_FOUND, 404, true);
  }
 ...
 * 활성 상태인 다른 플랜 그룹 조회
 */
async function _getActivePlanGroups(excludeGroupId?: string): Promise<Array<{ id: string; name: string | null }>> {
  const user = await getCurrentUser();
  if (!user || user.role !== "student") {
    throw new AppError("로그인이 필요합니다.", ErrorCode.UNAUTHORIZED, 401, true);
  }
