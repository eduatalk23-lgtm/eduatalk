...
export function LocationCheckOut({ onSuccess }: LocationCheckOutProps) {
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      const result = await checkOutWithLocation(latitude, longitude);
      if (result.success) {
        setSuccess(true);
        ...
        setError(result.error || "퇴실 체크에 실패했습니다.");
      }
    } catch (err: any) {
      let errorMessage = "위치 기반 퇴실 체크에 실패했습니다.";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xs text-gray-500">
        ...
        <p>• 학원 위치에서 설정된 반경 내에 있어야 퇴실이 인정됩니다.</p>
      </div>
    </div>
  );
}
